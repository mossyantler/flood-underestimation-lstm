...
### Step 6. Model 3는 “physics 전체를 LSTM이 조정”하는 구조보다 `bounded flux/state-constrained core`가 낫다

문헌을 종합하면, naive dynamic-parameter hybrid는 해석과 안정성에서 곤란해질 가능성이 높다. CAMELS 프로젝트에서는 `snow storage`, `soil storage`, `fast runoff`, `slow/baseflow`, `routing storage` 같은 최소 상태를 두고, LSTM이 모든 파라미터를 흔드는 대신 `melt`, `infiltration`, `percolation`, `routing coefficient` 정도만 제한적으로 제안하게 만드는 편이 안전하다.
...
이 subgroup 평가가 있어야 physics-guided hybrid의 incremental gain이 어디서 오는지 보인다.
...
현재 프로젝트의 질문은 “어떤 backbone이 더 센가”가 아니라, `왜 extreme flood peak가 눌리는가`, `output design만 바꿔도 좋아지는가`, `physics-guided core가 timing과 generalization에 추가 이득을 주는가`다. 이번 문헌 조사를 기준으로 보면, 첫 논문은 `multi-basin LSTM baseline -> probabilistic head -> bounded hybrid core`의 순서로 가는 게 가장 자연스럽다.
