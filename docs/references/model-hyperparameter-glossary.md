...
현재 broad config와 local safe config를 같이 놓고 보면, 우선순위가 높은 핵심 하이퍼파라미터는 아래 정도다.

| 항목 | 현재 broad 값 | 현재 safe 값 | 역할 |
| --- | --- | --- | --- |
| `seed` | `111` | `111` | 실험 재현성 |
| `hidden_size` | `128` | `64` | LSTM 내부 표현 용량 |
| `initial_forget_bias` | `3` | `3` | 초기 memory retention |
| `output_dropout` | `0.3` | `0.2` | regularization |
| `learning_rate` | `10^{-3} \to 5 \times 10^{-4} \to 10^{-4}` | `10^{-3} \to 5 \times 10^{-4} \to 10^{-4}` | optimizer step 크기 |
| `batch_size` | `256` | `64` | 한 번에 처리하는 샘플 수 |
| `epochs` | `30` | `20` | 전체 학습 반복 수 |
| `clip_gradient_norm` | `1` | `1` | gradient explosion 방지 |
| `seq_length` | `336` | `336` | 입력 문맥 길이 |
| `predict_last_n` | `24` | `24` | supervision 구간 길이 |
| `quantiles` | `[0.5, 0.9, 0.95, 0.99]` | 해당 없음 | probabilistic output 정의 |
| `quantile_loss_weights` | `[1.0, 1.0, 1.0, 1.0]` | 해당 없음 | quantile별 loss 비중 |
| `max_updates_per_epoch` | 해당 없음 | `200` | local safe run에서 epoch당 step 제한 |
...
`device`는 계산 장치를 뜻한다. 현재 broad/safe config 예시는 `mps`로 되어 있지만, 논문용 반복 실험에서는 CUDA GPU 환경을 기본으로 보는 것이 더 적절하다.

이 값은 성능 그 자체를 바꾸는 하이퍼파라미터는 아니지만, 실제로는 batch size, num_workers, cache 설정과 함께 실험 가능 범위를 크게 바꾸기 때문에 실험 조건 파라미터로 중요하다.

### 3.3 `num_workers`

`num_workers`는 DataLoader가 데이터를 병렬로 준비할 worker process 수다. broad config는 `4`, safe config는 `0`이다.

이 값이 커지면 데이터 로딩이 빨라질 수 있지만, 운영체제나 backend 조합에 따라 메모리 사용량이 급격히 커지거나 serialization overhead가 생길 수 있다. safe config에서 `0`으로 둔 것은 성능 최적화보다 안정성을 우선한 선택이다.
...
현재 broad config는 `128`, safe config는 `64`를 쓴다. 값이 커지면 모델이 더 복잡한 시계열 패턴과 basin-dependent interaction을 담을 수 있지만, 그만큼 파라미터 수와 메모리 사용량도 증가한다. 값이 작아지면 계산은 가벼워지지만, 표현력이 부족해져 underfitting이 생길 수 있다.
...
`output_dropout`은 head 앞에 적용하는 dropout 비율이다. broad config는 `0.3`, safe config는 `0.2`다.
...
현재 broad config에서 `0.3`을 쓰는 것은 모델 용량이 `hidden_size = 128`로 safe config보다 크기 때문에, regularization도 조금 더 주는 선택으로 읽을 수 있다.
...
`batch_size`는 한 번의 optimizer update 전에 몇 개의 학습 샘플을 묶어서 처리할지를 뜻한다. broad config는 `256`, safe config는 `64`다.
...
값이 커지면 gradient가 더 안정적이고 GPU 활용이 좋아질 수 있지만, 메모리 사용량이 증가한다. 값이 작아지면 메모리는 절약되지만 gradient noise가 커질 수 있다. safe config에서 `64`를 쓰는 것은 메모리 제약을 고려한 타협이다.

### 5.4 `epochs`

`epochs`는 학습 데이터를 몇 번 반복해서 볼지를 정하는 값이다. broad config는 `30`, safe config는 `20`이다.
...
`clip_gradient_norm`은 gradient explosion을 막기 위한 안전장치다. 현재 broad/safe 모두 `1`을 쓴다.
...
### 5.6 `max_updates_per_epoch`

`max_updates_per_epoch`는 safe config에만 들어 있는 값으로, 한 epoch 안에서 optimizer update 수를 강제로 제한한다. 현재 값은 `200`이다.

이 값은 “진짜 최적의 학습 규칙”이라기보다, 로컬 환경에서 메모리와 실행 시간을 통제하기 위한 안전장치에 가깝다. broad config에는 이 값이 없고 전체 데이터셋을 자연스럽게 한 바퀴 도는 구조다.
...
이 값은 validation 시 몇 개 basin을 랜덤하게 뽑아 평가할지를 정한다. broad config는 `200`, safe config는 `64`다.
...
validation 데이터를 메모리에 캐시할지 여부다. broad config는 `True`, safe config는 `False`다.

값을 `True`로 두면 validation이 빨라질 수 있지만 메모리 사용량이 증가한다. safe config에서 `False`로 둔 것은 메모리 절약이 우선이기 때문이다.
...
TensorBoard 로그를 남길지 여부다. broad config는 `True`, safe config는 `False`다.
...
test 시점의 full output을 저장할지 여부다. broad config는 `True`, safe config는 `False`다.
...
몇 epoch마다 가중치를 저장할지 정한다. broad config는 `1`, safe config는 `5`다.
...
| `max_updates_per_epoch` | 로컬 safe run에서 epoch당 step 수를 제한한다. |
