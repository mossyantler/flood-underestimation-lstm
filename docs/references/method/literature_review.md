...
현재 프로젝트의 핵심 문제는 `다른 환경의 유역`과 `극한 홍수`에서 수문 AI 모델이 peak를 낮게 예측하는 경향이다. 그래서 선행연구도 `누가 NSE가 높은가`보다 `왜 peak가 눌리는가`, `probabilistic output이 실제로 도움이 되는가`, `유역 일반화와 extreme-event extrapolation에 무엇이 중요한가`를 중심으로 읽어야 한다.
...
| physics-guided modeling | `MC-LSTM` (2021), interpretable LSTM hydrology (2024), differentiable hydrologic models (2023) | future work에서 physics-guided core를 state/flux-constrained 방향으로 설계할 근거 | Discussion, future work |
| hybrid critique | `To bucket or not to bucket?` (2024), `When physics gets in the way` (2026) | naive dynamic-parameter hybrid의 한계와 `AI가 physics를 덮어쓸 수 있음`을 보여줌 | future-work scope delimitation |
...
`MC-LSTM`, interpretable LSTM, differentiable hydrologic model 연구는 `physics를 구조 안에 실제로 넣는 방식`을 고민하게 만든다. 다만 현재 논문은 이 축을 메인 비교축으로 채택하지 않고, `future work에서 어떤 형태의 core가 더 타당한가`를 정리하는 참고 축으로 둔다.
...
|---|---|---|
| `To bucket or not to bucket?` (2024) | naive dynamic-parameter hybrid는 성능이나 해석 모두 애매할 수 있다 | 현재 논문에서 Model 3를 메인 비교축으로 두지 않는 근거가 된다 |
| `When physics gets in the way` (2026) | physics constraint가 LSTM의 부담을 줄이기보다 오히려 보정 부담을 늘릴 수 있다 | future work에서도 `AI가 physics를 덮어쓰는 구조`를 피해야 한다 |
| `A national-scale hybrid model for enhanced streamflow estimation` (2024) | physics-aware input은 도움이 되지만 peak underestimation을 끝내지는 못한다 | probabilistic head 이후에도 남는 한계를 future work에서 논의할 근거다 |
...
2. physics-guided conceptual core는 흥미로운 방향이지만, 현재 논문에서는 `future work`로 두는 편이 더 설득력 있다.
3. 실험은 `temporal split`만으로 끝내지 말고 `basin holdout`과 `extreme-event holdout`을 함께 둬야 한다.
4. 첫 논문에서는 backbone을 `multi-basin LSTM`으로 고정하고, output design의 효과를 가장 먼저 분리해서 보는 편이 가장 설득력 있다.

related work 서술 순서는 `CAMELS / large-sample hydrology -> LSTM baseline과 multi-basin regionalization -> probabilistic tail modeling과 extrapolation 한계 -> physics-guided / hybrid critique` 정도로 두면 현재 프로젝트 질문과 가장 자연스럽게 이어진다.

## 관련 문서

- [`../../experiment/method/model/design.md`](../../experiment/method/model/design.md): 현재 연구 질문과 비교축
- [`../../experiment/method/model/architecture.md`](../../experiment/method/model/architecture.md): 현재 논문 범위의 구조와 future-work 메모
- [`../../experiment/method/model/experiment_protocol.md`](../../experiment/method/model/experiment_protocol.md): 실제 실험 규칙
