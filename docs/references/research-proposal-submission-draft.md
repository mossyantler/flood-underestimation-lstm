...
이 문서는 공식 실험 규칙의 source of truth가 아니다. 실험 규칙과 구현 기준은 [`../research/design.md`](../research/design.md), [`../research/architecture.md`](../research/architecture.md), [`../research/experiment_protocol.md`](../research/experiment_protocol.md)를 우선한다.
...
Probabilistic Head와 Physics-Guided Hybrid 확장을 통한 Multi-Basin LSTM의 극한 홍수 첨두 과소추정 완화
...
본 연구는 이 문제를 단일 모델 개선이 아니라 `세 단계 비교 구조`로 접근한다. 첫 번째 단계는 deterministic multi-basin LSTM baseline이다. 두 번째 단계는 backbone은 그대로 두고 output head만 probabilistic quantile head로 바꾼 모델이다. 세 번째 단계는 probabilistic head 위에 physics-guided conceptual core를 추가한 hybrid 모델이다.

이와 같은 비교 구조를 채택하는 이유는 성능 향상의 원인을 분리하기 위해서다. Model 1과 Model 2의 차이는 `output design`의 효과로, Model 2와 Model 3의 차이는 `physics-guided structure`의 추가 효과로 해석할 수 있다. 따라서 본 연구는 단순 성능 비교를 넘어서, 극한 홍수 첨두 과소추정이 왜 발생하며 어떤 구조적 수정이 이를 줄일 수 있는지를 단계적으로 설명하는 연구다.
...
본 연구는 이 비판을 수용하여, Model 3를 `dynamic-parameter shell`이 아닌 `state/flux-constrained conceptual core` 방향으로 설계한다. 즉 LSTM이 physics를 완전히 대체하는 것이 아니라, 제한된 flux와 bounded coefficient를 제안하고 storage 및 routing의 뼈대는 conceptual core가 담당하도록 한다.

### 3.5 본 연구의 차별성

기존 연구들이 각각 baseline, probabilistic prediction, hybrid modeling, generalization 평가를 다루기는 했지만, 동일한 backbone과 동일한 split 위에서 `deterministic output -> probabilistic output -> physics-guided hybrid`를 단계 비교하여 peak underestimation 원인을 분해한 연구는 드물다.

본 연구의 차별성은 크게 세 가지다. 첫째, CAMELSH hourly를 사용하여 flood peak magnitude와 timing을 직접 평가한다. 둘째, DRBC regional holdout과 extreme-event holdout을 함께 둔다. 셋째, Model 1 대비 Model 2는 output design 효과, Model 2 대비 Model 3는 physics-guided 효과로 해석할 수 있게 비교축을 설계한다.
...
셋째, probabilistic baseline 위에 physics-guided conceptual core를 추가하여, peak magnitude뿐 아니라 timing과 basin generalization까지 추가 개선이 가능한지 확인한다.
...
3. Physics-guided conceptual core는 특히 routing, storage memory, snow- or groundwater-influenced basin에서 추가 이득을 줄 수 있다.
...
Deterministic Model 1의 출력은 각 시간 스텝의 point estimate $\hat{Q}_t$다. Probabilistic Model 2와 Model 3의 출력은 $q_{0.50}$, $q_{0.90}$, $q_{0.95}$, $q_{0.99}$의 quantile streamflow sequence다. 이때 point metric 계산에는 $q_{0.50}$을 중심 예측선으로 사용한다.
...
본 연구는 세 종류의 split을 공식 비교축으로 둔다.
...
이 구조의 목적은 backbone을 고정한 채 output design과 physics-guided structure의 효과를 단계적으로 분리하는 데 있다.
...
### 6.4 Model 3: Physics-guided probabilistic hybrid

Model 3는 probabilistic LSTM 위에 conceptual core를 추가한다.
...
flowchart TD
  ...
  J --> K["Quantile streamflow output"]
```

이 모델은 아직 후속 설계와 구현이 필요한 부분이 있다. 다만 첫 논문 수준에서는 `snow storage`, `soil storage`, `fast runoff storage`, `slow/baseflow storage`, 필요 시 `routing storage`를 state variable의 기본 골격으로 두는 것이 적절하다.
...
Model 3는 probabilistic loss에 physics regularization을 추가하는 구조를 목표로 한다.

$$
L_{\mathrm{total}}
=
L_{\mathrm{prob}}
+ \lambda_{\mathrm{mass}} L_{\mathrm{mass\_balance}}
+ \lambda_{\mathrm{nonneg}} L_{\mathrm{nonnegativity}}
+ \lambda_{\mathrm{bound}} L_{\mathrm{storage\_bounds}}
$$
...
Model 2 대비 Model 3의 차이는 `physics-guided state/routing structure의 추가 효과`로 해석한다. 만약 timing과 basin holdout 성능이 더 좋아진다면, conceptual core가 tail-aware output만으로 부족한 부분을 보완했다고 해석할 수 있다.

---

## 8. 기대효과 및 학술적 기여

본 연구의 첫 번째 기대효과는 `홍수 첨두 과소추정 문제를 구조적으로 분해해 설명할 수 있다`는 점이다. 많은 기존 연구는 hybrid 구조를 제안하면서도, 성능 개선의 원인이 output design인지 physics 구조인지 분리해 보여주지 못했다. 본 연구는 세 단계 비교 구조를 통해 그 원인을 더 명확하게 해석할 수 있다.
...
세 번째 기대효과는 `physics-guided hybrid의 설계 방향에 대한 실증적 단서`를 제공한다는 점이다. 만약 probabilistic head만으로 큰 개선이 가능하다면, 복잡한 hybrid를 무조건 먼저 설계할 필요가 없다는 결론이 가능하다. 반대로 Model 3가 timing과 basin generalization에서 뚜렷한 이득을 보인다면, state/flux-constrained hybrid의 필요성을 더 강하게 주장할 수 있다.
...
- flux / bounded-coefficient head 구현
- physics regularization 안정화
...
Model 3와 multi-seed sensitivity, post-processing까지 함께 고려하면 `24GB급 GPU 1장 + RAM 128GB + NVMe 2TB` 수준이 가장 실용적이다. 더 큰 규모의 실험을 위해서는 A100 40GB 이상의 서버 환경이 유리할 수 있다.
...
현재 단계에서 가장 중요한 점은 backbone을 고정한 상태에서 `deterministic -> probabilistic -> physics-guided` 비교를 수행한다는 실험 철학이다. 이는 peak underestimation 완화의 원인을 구조적으로 분해해 설명하기 위한 설계다.

## 관련 문서

- [`research-plan-extreme-flood-underestimation.md`](research-plan-extreme-flood-underestimation.md): 설명형 상세 연구계획서 초안
- [`../research/design.md`](../research/design.md): 공식 연구 질문과 비교축
- [`../research/architecture.md`](../research/architecture.md): 세 모델의 구조 설명
- [`../research/experiment_protocol.md`](../research/experiment_protocol.md): split, loss, metric의 공식 규칙
