...
seed override가 필요하면 broad config를 임시로 복사해 seed만 바꾼 temporary override config를 만들어 반복 실행하면 된다. 현재 저장소에는 reference broad runner [`../../../../scripts/runs/official/run_broad_multiseed.sh`](../../../../scripts/runs/official/run_broad_multiseed.sh), 현재 채택된 fixed `subset300` main comparison runner [`../../../../scripts/runs/official/run_subset300_multiseed.sh`](../../../../scripts/runs/official/run_subset300_multiseed.sh), 로컬 점검용 실행 스크립트 [`../../../../scripts/runs/dev/run_local_sanity.sh`](../../../../scripts/runs/dev/run_local_sanity.sh)가 있다. deterministic scaling pilot은 별도 경로 [`../../../../scripts/runs/pilot/run_deterministic_scaling_pilot.sh`](../../../../scripts/runs/pilot/run_deterministic_scaling_pilot.sh)로 분리해 두고, 본 3-repeat main comparison runner와 섞지 않는다.
...
### 1.5 Subset300 test checkpoint 규칙

DRBC test는 두 층으로 읽는다. 공식 보고용 primary checkpoint는 `test`를 보기 전에 non-DRBC validation metric만으로 잠그고, 별도의 full checkpoint sweep은 sensitivity / diagnostic 결과로 둔다.

Model 1의 training loss는 `nse`이고 Model 2의 training loss는 `pinball`이라서 validation loss를 그대로 비교 기준으로 쓰면 안 된다. 두 모델에 공통으로 정의되는 built-in metric인 `validation median NSE`가 현재 가장 무난한 checkpoint selection rule이다. 현재 subset300 validation CSV 기준 primary checkpoint는 아래처럼 둔다.

| model | seed | primary epoch | selection rule |
| --- | ---: | ---: | --- |
| Model 1 | 111 | 25 | validation median NSE best |
| Model 1 | 222 | 10 | validation median NSE best |
| Model 1 | 444 | 15 | validation median NSE best |
| Model 2 | 111 | 5 | validation median NSE best |
| Model 2 | 222 | 10 | validation median NSE best |
| Model 2 | 444 | 10 | validation median NSE best |
...
같은 validation checkpoint grid는 극한호우 historical stress test에도 별도 sensitivity run으로 적용할 수 있다. 이때는 `output/model_analysis/extreme_rain/all/`에 primary stress test와 분리해 저장하고, Model 1/2 same-epoch pair별 `q50/q90/q95/q99` peak tracking과 false-positive tradeoff가 checkpoint 선택에 민감한지만 본다.
...
### 1.6 Deterministic scaling pilot의 위치
...
요약 수치는 `output/basin/all/screening/splits/drbc_holdout/drbc_holdout_split_summary.json`에 정리한다.
...
현재 본 실험의 공식 train/validation basin file은 위 tracked subset 중 `pilot 300`만 사용한다. 즉 train `269`, validation `31`, test `38` 구성을 Model 1 / Model 2 seed `111 / 222 / 444`에 공통으로 고정하고, 공식 실행 진입점은 [`../../../../scripts/runs/official/run_subset300_multiseed.sh`](../../../../scripts/runs/official/run_subset300_multiseed.sh)다. broad prepared split 전체 `1705 / 198 / 38`은 subset 생성의 source pool과 reference architecture context로 유지하되, 현재 compute-constrained main comparison의 직접 실행 split은 아니다.
...
Extreme-event holdout은 `훈련 분포 바깥의 large observed streamflow peak`에 대한 외삽 성능을 보기 위한 split이다. 이 split은 반드시 [`event_response_spec.md`](../basin/event_response_spec.md)를 기준으로 event candidate를 정의한 뒤 만들어야 한다. 여기서 event candidate는 official flood inventory가 아니라 basin-specific observed high-flow response다.
...
결과 해석에서는 `selected_threshold_quantile`, `flood_relevance_tier`, `return_period_confidence_flag`를 반드시 보존한다. 특히 Q95 fallback event와 Q99 event를 같은 severity로 해석하지 않고, return-period proxy도 공식 NOAA/USGS flood frequency가 아니라 CAMELSH hourly annual-maxima proxy로 표기한다.

event typing을 사용한 subgroup 결과는 main split definition이 아니라 post-hoc stratified analysis다. 따라서 type별 결론을 본문에 쓸 때는 `Q99-only`, fallback 포함 전체, `uncertain_high_flow_candidate` 포함/제외, return-period confidence flag별 결과가 같은 방향인지 함께 확인한다. snow-related label은 1°C degree-day snowmelt proxy이므로 confirmed snowmelt occurrence로 쓰지 않는다.

### 3.4 Minimum quality gate와 usability gate

현재 broad profile의 공식 basin 추적 표는 `output/basin/all/screening/camelsh_basin_master_checklist_broad.csv`다. 이 표는 CAMELSH 전체 9008 basin에 대해 `minimum quality gate`와 `split-level usability gate`를 같은 기준으로 기록한다.
...
### 3.5 Missing-data handling과 실제 sample 사용 규칙

현재 subset300 main comparison은 `GenericDataset`을 사용하므로 결측값은 NetCDF 안에서 반드시 `NaN`으로 표시되어야 한다. `-999` 같은 sentinel 값이 남아 있으면 framework가 결측으로 인식하지 못한다.

Training sample은 `seq_length = 336` 시간 입력 window와 `predict_last_n = 24` 시간 target window를 기준으로 판정된다. Training에서는 `seq_length` 안의 dynamic input 중 하나라도 `NaN`이면 해당 sample 전체가 lookup table에서 제외된다. Static attribute에 `NaN`이 있어도 sample은 invalid 처리된다. Target `Streamflow`는 조금 다르다. `predict_last_n` 전체가 모두 `NaN`이면 sample이 제외되지만, 일부 시간만 `NaN`이면 sample은 유지되고 loss 계산에서 해당 target 시간만 mask된다. Model 1의 `nse` loss와 Model 2의 `pinball` loss 모두 target `NaN`을 mask한다.

Validation/test에서는 충분한 history가 있으면 forward pass는 더 넓게 수행한다. 이후 metric 계산에서는 관측 `Streamflow` 또는 simulation이 `NaN`인 timestep을 제외한다. 따라서 test metric은 split 기간의 모든 달력 시간이 아니라, 관측과 예측이 모두 유효한 timestep 위에서 계산된다. 이 차이를 보고할 때는 `calendar period`, `valid target coverage`, `metric support`를 구분한다.

현재 subset300 split에서 확인한 결과, config의 11개 dynamic input(`Rainf`, `Tair`, `PotEvap`, `SWdown`, `Qair`, `PSurf`, `Wind_E`, `Wind_N`, `LWdown`, `CAPE`, `CRainf_frac`)은 train/validation/test split window 안에서 결측 시간이 없었다. 선택된 static attributes도 결측이 없었다. 즉 현재 실행에서 sample/metric support를 줄이는 주된 원인은 forcing이나 static attribute 결측이 아니라 `Streamflow` target 결측이다. Daily target coverage chart와 요약 CSV는 [`../../../../output/basin/timeseries/`](../../../../output/basin/timeseries/) 아래에 생성한다.

현재 실험은 이미 완료된 compute-constrained main comparison이므로, split membership을 사후에 바꾸거나 재학습하지 않는다. 대신 `selected basin`, `split-period eligibility`, `effective sample support`를 구분해서 보고한다. 예를 들어 train basin은 split 파일에 포함되었고 split-period minimum valid target count를 통과했으면 eligible basin으로 유지한다. 다만 해당 basin이 실제 학습에 기여한 정도는 `valid_model_sample_count`와 `used_coverage_fraction`으로 별도 진단한다. 이 진단은 [`../../../../output/basin/timeseries/train_support_diagnostics.csv`](../../../../output/basin/timeseries/train_support_diagnostics.csv)와 [`../../../../output/basin/timeseries/train_support_diagnostics_summary.json`](../../../../output/basin/timeseries/train_support_diagnostics_summary.json)에 둔다.

이 구분은 특히 sparse train-support basin을 설명할 때 중요하다. 현재 fixed `scaling_300` train split에서 `used_coverage_fraction <= 0.10`인 basin은 `03057000` 하나이며, 이 basin의 estimated valid training sample은 `2,180`개로 전체 train valid sample `21,017,096`개의 약 `0.0104%`다. 따라서 이 basin은 train membership에는 포함되지만, model fitting에 대한 sample-level 영향은 매우 작다. Model 1과 Model 2는 동일한 train split과 동일한 coverage pattern을 공유하므로, 이 문제는 두 모델 간 paired comparison의 공정성을 깨는 요인이라기보다 data-support limitation으로 보고해야 한다.
...
`top 1% flow recall`과 `event-level RMSE`는 현재 built-in이 아니므로 custom post-processing으로 계산한다. 특히 `event-level RMSE`는 event 정의가 [`event_response_spec.md`](../basin/event_response_spec.md)와 일치해야 하므로, event table 기반 후처리로 계산하는 것을 원칙으로 한다.
...
현재 vendored NeuralHydrology에는 coverage/calibration built-in metric이 없으므로, `test_all_output.p` 또는 후속 export 결과를 읽는 custom evaluation script에서 계산한다. 다만 `save_all_output=True`는 `y_quantiles`뿐 아니라 recurrent state와 full sequence output까지 저장할 수 있어서, 모든 seed/model/validation epoch sweep에 일괄 적용하면 저장 공간과 I/O 비용이 커진다. 따라서 full-period DRBC validation-epoch test sweep은 우선 `save_all_output=False`인 built-in q50 중심 결과로 돌리고, `q90/q95/q99` coverage/calibration은 primary checkpoint와 필요한 robustness checkpoint만 별도 export 또는 lean custom evaluator로 계산한다. 극한호우 stress test는 full-period dump가 아니라 event inference block에 대한 lean export이므로, all-validation-epoch sensitivity에서도 필요한 `q50/q90/q95/q99`만 저장한다.
...
- `test/.../test_all_output.p`: probabilistic head를 포함한 full output. 저장 비용이 크므로 모든 checkpoint sweep의 필수 산출물이 아니라 selected checkpoint용 선택 산출물로 둔다.
...
- [`../basin/event_response_spec.md`](../basin/event_response_spec.md): extreme-event 정의와 event descriptor 규칙
