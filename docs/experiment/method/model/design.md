...
CAMELS-US local dataset은 현재 공식 데이터 의존성에서 제외한다. CAMELS 계열 다른 데이터셋은 related work나 후속 외부 robustness check 문맥에서만 언급한다.
...
별도 center loss를 두지 않고, `q50`을 포함한 모든 quantile에 대해 weighted `pinball loss`를 계산한다.

공식 loss는 LaTeX 표기로 아래처럼 쓴다.

$$
\mathcal{L}_{\mathrm{M2}}
=
\frac{1}{|\Omega|}
\sum_{(i,t)\in\Omega}
\frac{
\sum_{k=1}^{K} w_k \rho_{\tau_k}\left(e_{i,t,k}\right)
}{
\sum_{k=1}^{K} w_k
},
\qquad
\tau_k \in \{0.5, 0.9, 0.95, 0.99\}
$$

자세히 풀면, 먼저 각 basin-sample `i`, 시간 `t`, quantile `k`에 대해 관측 유량과 예측 quantile의 차이를 아래처럼 둔다.
...
각 quantile의 pinball loss 항은 다음과 같다.

$$
\rho_{\tau_k}\left(e_{i,t,k}\right)
=
\max\left(
\tau_k e_{i,t,k},
\left(\tau_k - 1\right)e_{i,t,k}
\right)
$$

같은 식을 오차 방향별로 쓰면 아래와 같다.

$$
\rho_{\tau_k}\left(e_{i,t,k}\right)
=
\begin{cases}
\tau_k\left(y_{i,t} - \hat{q}_{i,t}^{(\tau_k)}\right),
& y_{i,t} \ge \hat{q}_{i,t}^{(\tau_k)} \\
\left(1-\tau_k\right)\left(\hat{q}_{i,t}^{(\tau_k)} - y_{i,t}\right),
& y_{i,t} < \hat{q}_{i,t}^{(\tau_k)}
\end{cases}
$$

한 시점의 quantile loss는 quantile별 pinball loss를 가중 평균한 값이다.
...
최종 loss는 관측값이 유효한 모든 basin-sample/time pair 집합 $\Omega$에 대해 평균낸다.
...
현재 main comparison에서는 모든 quantile weight를 동일하게 두므로 $w_k = 1$이다.

핵심은 `q50`을 중앙 예측선으로 쓰되, 이를 별도 deterministic loss가 아니라 quantile objective 안에서 함께 학습한다는 점이다. `q90`, `q95`, `q99`는 같은 objective 안에서 upper tail을 직접 학습한다.
...
- [`../../../references/method/literature_review.md`](../../../references/method/literature_review.md): 관련 문헌 배경
