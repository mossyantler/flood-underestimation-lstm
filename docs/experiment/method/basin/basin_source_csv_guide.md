...
현재 기준 분석 테이블은 [`drbc_selected_basin_analysis_table.csv`](../../../../output/basin/drbc/analysis/basin_attributes/tables/drbc_selected_basin_analysis_table.csv)이며, 아래 8개 source CSV를 병합해 만든다.

| # | Source CSV | 역할 |
|---|---|---|
| 1 | [`camelsh_drbc_selected.csv`](../../../../output/basin/drbc/basin_define/camelsh_drbc_selected.csv) | basin selection / 행 정의 |
...
[`camelsh_drbc_selected.csv`](../../../../output/basin/drbc/basin_define/camelsh_drbc_selected.csv)는 attribute 파일이 아니라 분석 테이블의 **행(row)을 결정하는 entry table**이다.
...
`SLOPE_PCT`는 flood-relevant screening에서 가장 직관적인 구조 변수 중 하나다. 다른 조건이 비슷하다면 slope가 큰 basin이 peak response도 더 빠른 경향이 있다.
...
실무적으로는 `high_prec_freq`, `high_prec_dur`, `frac_snow`, `p_seasonality`를 우선 확인한다. flood-relevant candidate basin은 큰 비가 자주 오거나, snowmelt proxy가 관여하거나, 계절 집중도가 높은 경우가 많다.
...
[`drbc_selected_basin_analysis_table.csv`](../../../../output/basin/drbc/analysis/basin_attributes/tables/drbc_selected_basin_analysis_table.csv)는 다음 순서로 읽는다.
...
**hydro 변수는 검증 역할이다.**
land cover나 slope가 빠른 응답을 시사하더라도, `BFI_AVE`나 streamflow 응답 지표가 뒷받침되지 않으면 flood-relevant basin으로 단정해서는 안 된다. 정적 변수는 *설명*, hydro 변수는 *검증*이다.
...
정적 변수는 basin의 구조적 배경과 해석 단서를 제공한다. 최종 flood-relevant basin 선정은 이 정적 정보 위에 품질 게이트와 observed-flow 지표를 더해 진행해야 한다. 이때 observed-flow event는 official flood inventory가 아니라 high-flow event candidate라는 점을 함께 둔다.
