...
3. `annual peaks, Q99 high-flow candidate frequency, RBI 같은 observed-flow metric 기반 flood-relevant basin 선정`
4. `hydromod risk 여부에 따라 broad / natural cohort 분리`

이 구조를 쓰는 이유는 단순하다. 연구권역을 고정하고, 데이터 품질을 보장하고, 실제 observed high-flow / flood-like response를 확인한 뒤, anthropogenic disturbance를 분리해 보기 위해서다.
...
둘째는 `Q99 high-flow candidate frequency`다. usable year당 Q99-level observed high-flow event candidate가 얼마나 자주 나타나는지 본다.

$$
F_i^{99} = \frac{N_i^{99}}{Y_i^{\mathrm{usable}}}
$$

여기서 \(N_i^{99}\)는 basin \(i\)에서 Q99 threshold를 초과한 독립 observed high-flow event candidate의 개수다.
...
observed-flow metric도 sample이 충분해야 의미가 있다. 따라서 flood relevance를 판정하기 전, annual peak와 extreme-flow event candidate sample이 너무 적은 basin은 제외한다.
...
`Q99 high-flow candidate frequency`는 extreme high-flow response가 실제로 얼마나 자주 나타나는지를 보여준다. 이는 단순히 peak가 한 번 컸는지보다, basin이 반복적으로 extreme response를 보이는지를 판단하는 데 유용하다. 다만 Q99 candidate 자체는 official flood occurrence가 아니므로, 최종 표에서는 `selected_threshold_quantile`과 `flood_relevance_tier`를 같이 남긴다.
...
Observed-flow screening의 핵심 점수는 여전히 `annual peak specific discharge`, `Q99 high-flow candidate frequency`, `RBI`를 중심으로 둔다. 다만 basin 해석과 event response table의 맥락 설명을 강화하기 위해, 재현기간별 강수량과 홍수량을 `reference descriptor`로 추가한다.
