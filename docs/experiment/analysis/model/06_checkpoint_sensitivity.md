...
Hydrograph all-epoch 분석에서는 q95/q99의 upper-tail 효과가 반복적으로 나타난다. 특히 `q99`는 여러 same-epoch run에서 Model 1 대비 top-1% underestimation fraction을 낮추는 방향을 보인다. 다만 epoch가 뒤로 갈수록 quantile band가 좁아지는 경향이 있어, tail safety margin이 줄 수 있다.
...
논문용으로는 primary result가 all-epoch distribution의 어디에 위치하는지 보여주는 compact figure가 필요하다. 예를 들어 `top1 underestimation fraction`, `q99-q50 spread`, `positive-response under-deficit reduction`, `negative-control false-positive proxy`를 한 figure에 요약할 수 있다.
