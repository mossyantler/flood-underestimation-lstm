...
| 2 | [`02_primary_high_flow_peak_performance.md`](02_primary_high_flow_peak_performance.md) | 완료에 가까움 | top-flow stratum과 observed peak hour에서 `q90/q95/q99`가 peak underestimation을 줄이는지 본다. |
...
| 5 | [`05_extreme_rain_stress_test.md`](05_extreme_rain_stress_test.md) | 완료에 가까움 | hourly `Rainf` 기반 historical stress event에서 upper quantile output의 peak tracking과 false-positive tradeoff를 본다. |
...
| 7 | [`07_broad_vs_natural_robustness.md`](07_broad_vs_natural_robustness.md) | 예정 | Natural subset robustness 결과는 아직 model-output 분석 산출물이 없으므로 계획만 둔다. |
