...
- Per-basin τ-monotonicity violation rate < 20% (B3 결과: Q99 scope 0%, NOAA scope 0%).
...
| τ | α (basin median) | β (basin median) |
| --- | --- | --- |
| q50 | 0.788 | 0.282 |
| q90 | 0.482 | 0.566 |
| q95 | 0.404 | 0.677 |
...
NOAA confirmed flood event-set 위에서도 q99이 peak deficit 17% / window capture 98% — Q99 scope 결과와 일관.
...
- per-basin monotonicity는 quantile crossing 가능성으로 인해 100% 보장되지 않으나 본 결과는 0% 위반.
