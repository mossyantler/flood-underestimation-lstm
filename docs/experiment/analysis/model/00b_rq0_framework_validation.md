...
관측 위치 구간(`obs_class`)은 사건 첨두 시각의 관측 유량이 Model 2의 `q50/q90/q95/q99` 예측 사다리 어디에 놓이는지로 계산한다. Q99 사건과 NOAA 사건은 seed별 예측선이 다르므로 `(basin_id, seed, peak_time)`마다 `obs_class`를 계산한다. 사건 하나를 단일 판정으로 요약해야 하는 상관관계 표에서는 seed별 `obs_class` 번호의 **중앙값**을 primary target으로 쓴다. `obs_class`는 0~4의 순서형 구간이라 평균값이 실제 구간을 뜻하지 않기 때문이다. 다만 seed 평균(`oc_seed_mean`), 최소·최대, 표준편차는 보조 컬럼으로 남겨 seed 민감도를 확인한다. 실제로 Q99 사건은 926건 중 613건(66.2%), NOAA/NWS 사건은 65건 중 44건(67.7%)에서 세 seed의 위치 구간이 완전히 같지 않다. 따라서 단일값을 만들 때 중앙값을 쓰되, 평균과 범위도 같이 남겨야 한다. 전체 강우 사건은 세 seed의 `obs/q50/q90/q95/q99`를 먼저 `(basin_id, datetime)` 단위로 평균한 뒤 한 사건당 하나의 `obs_class`를 만든다.
...
이후 `compute_ub_location_class.py`가 Q99 사건의 `peak_time`을 각 seed의 `required_series.csv`와 붙여 `obs_class`를 만든다. Q99 사건 926건은 seed 3개와 결합되므로 원래 위치 구간 표는 2,778개 `(event, seed)` 행을 갖는다. 사건 단위 상관관계 분석에서는 같은 `(basin_id, peak_time)`의 seed별 위치 번호를 중앙값으로 묶어 하나의 target을 만들고, `oc_seed_mean/min/max/std`를 함께 저장한다.
...
이후 Q99 사건과 동일하게 `compute_ub_location_class.py`가 각 seed의 `required_series.csv`에서 사건 첨두 시각의 `obs/q50/q90/q95/q99`를 가져와 `obs_class`를 계산한다. NOAA 범위 65건은 seed 3개와 결합돼 원래 위치 구간 표가 195개 `(event, seed)` 행을 갖고, 상관관계 표에서는 사건 단위로 seed별 위치 번호의 중앙값을 primary target으로 쓴다.
...
- Q99 초과 사건의 약 **47%에서 관측이 q99마저 초과**한다(유역 중앙값). 반대로 말하면 가장 보수적인 예측선인 `q99`가 Q99 이상 고유량 첨두의 절반 이상을 밴드 안으로 끌어들였다는 뜻이기도 하다.
- NOAA/NWS catalog 홍수 범위는 유역 중앙값 기준 **100%가 `above_q99`**다. 사회적으로 확인된 홍수 첨두에서는 모델이 매우 일관되게 과소추정한다.

단순히 "덮었는가"만으로는 부족하다. `q99`가 덮은 사건에서는 얼마나 크게 넘겨 덮었는지, 못 덮은 사건에서는 얼마나 크게 부족했는지도 같이 봐야 한다.

| 범위 | `q99` 판정 | 상대 크기 지표 | 유역 중앙값 [IQR] | 의미 |
| --- | --- | ---: | ---: | --- |
| Q99 사건 | 못 덮음 (`above_q99`) | 상대 부족 폭 `(obs−q99)/obs` | **0.378** [0.294 / 0.474] | 못 덮은 사건에서 `q99`는 관측 첨두보다 전형적으로 38% 낮다 |
| Q99 사건 | 덮음 (`≤q99`) | 상대 초과 폭 `(q99−obs)/obs` | **0.544** [0.342 / 1.170] | 덮은 사건에서는 안전 여유가 전형적으로 관측 첨두의 54% 수준이다 |
| NOAA/NWS | 못 덮음 (`above_q99`) | 상대 부족 폭 | **0.344** [0.208 / 0.445] | 사회적으로 확인된 홍수에서 못 덮을 때도 부족 폭이 크다 |
| NOAA/NWS | 덮음 (`≤q99`) | 상대 초과 폭 | **0.810** [0.550 / 1.036] | 일부 seed·사건에서는 크게 보수적으로 덮는다 |

따라서 Q99 사건에서 `q99`의 47% 미포착은 "실패율"만이 아니다. `q99`는 Q99 tail의 절반 이상을 덮지만, 못 덮을 때는 관측 첨두 대비 약 38% 낮고, 덮을 때는 약 54%의 보수 여유를 갖는다. 즉 **포착 여부와 포착 크기 사이의 맞교환**이 핵심이다.

밴드 안에서 τ를 올릴수록 전체 사건 기준 과소추정 폭(under_gap)은 줄지만 과대추정 폭(over_gap)이 커지는 맞교환도 보인다(Q99 사건, 유역 중앙값):
...
전체 사건을 함께 놓고 보면 q99에서 전형적 과소추정 폭은 거의 닫히지만(0.018), 조건부로 `above_q99`만 떼어 보면 부족 폭은 여전히 크다(상대 부족 폭 중앙값 0.378). 이것이 **RQ-2(위쪽 분위로 첨두 과소추정 완화)의 정량적 문제 정의**다.
...
| **CAPE** | +0.03 | +0.22 | +0.13 | 대류 잠재력. 모든 체제 양수 |
| 관측 첨두 obs_peak (누수 기준선 L) | +0.46 | +0.44 | — | 사용 불가. area(+0.50)가 이에 맞먹거나 더 큼 |
...
| q99_q50_ratio | −0.14 | −0.16 | +0.01 | 전 범위 붕괴 |
| rain_sum(강우 총량) | −0.24 | +0.08 | **−0.03** | 전 범위 ≈0, NOAA에서는 안정 없음 |
| rain_max_1h·NWS 등급 | −0.20 | +0.05 | ≈0 | 전 범위 ≈0 |

- 밴드 폭이 넓을수록 obs가 그 안에 잡힐 확률이 높아 obs_class가 낮아지는 것은 **정의상 음의 결합**이지 예측력이 아니다.
- 강우 총량은 Q99 사건에서는 음수지만 NOAA/NWS와 전체 강우에서는 안정적으로 같은 방향을 유지하지 않는다. 비가 많으면 LSTM이 예측·밴드를 올려 상단이 높아질 수 있으므로, 단순 강우량 상관은 **모델 반응을 통한 간접 결합** 또는 범위 제한으로 해석한다.
...
- **과소추정의 정량 규모**: 가장 보수적인 `q99`는 Q99 사건의 절반 이상을 덮지만, 유역 중앙값 기준 약 47%는 여전히 덮지 못한다. 못 덮은 Q99 사건의 상대 부족 폭 중앙값은 0.378이고, 덮은 사건의 상대 초과 폭 중앙값은 0.544다. 따라서 결론은 “덮었는가”가 아니라 “얼마나 부족했고, 얼마나 보수적으로 덮었는가”까지 포함해야 한다.
