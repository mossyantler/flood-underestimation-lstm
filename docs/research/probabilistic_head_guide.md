...
- physics-guided hybrid 구조 자체

## 상세 서술

즉 이 문서는 `개념 설명`이 목적이다. 실제 key와 metric 규칙은 [`../research/experiment_protocol.md`](../research/experiment_protocol.md), 현재 논문 범위의 구조는 [`../research/architecture.md`](../research/architecture.md)에서 다룬다.
...
이 프로젝트에서 probabilistic head를 먼저 넣으려는 이유도 여기 있다. 현재 논문은 `출력 설계만 바꿔도 peak bias가 줄어드는지`를 먼저 확인하려는 것이다.
...
`output head만 바꾸면 extreme flood underestimation이 줄어드는가?`
...
Model 2:
inputs -> LSTM -> probabilistic head -> q50, q90, q95, q99
```
...
아니다. 이 단계는 어디까지나 `output design`을 바꾸는 것이다. physics-guided 구조는 현재 논문 범위 밖의 future work다.
...
6. 현재 논문의 메인 비교축은 `Model 1 vs Model 2`다.

한 문장으로 다시 정리하면, probabilistic head는 `유량 하나만 찍는 구조`를 `중심선과 상위 홍수 범위를 함께 예측하는 구조`로 바꿔서, extreme flood peak를 더 직접적으로 다루려는 장치다.

## 관련 문서

- 현재 논문 범위의 구조는 [`../research/architecture.md`](../research/architecture.md)에서 다룬다.
- 모델 비교 질문과 contribution 구조는 [`../research/design.md`](../research/design.md)에서 다룬다.
- config key와 평가 규칙은 [`../research/experiment_protocol.md`](../research/experiment_protocol.md)에서 다룬다.
