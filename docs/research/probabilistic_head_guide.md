...
즉 이 문서는 `개념 설명`이 목적이다. 실제 key와 metric 규칙은 [`experiment_protocol.md`](experiment_protocol.md), 세 모델 전체 구조는 [`architecture.md`](architecture.md)에서 다룬다.
...
- 세 모델의 전체 구조는 [`architecture.md`](architecture.md)에서 다룬다.
