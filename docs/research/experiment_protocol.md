...
seed override가 필요하면 broad config를 임시로 복사해 seed만 바꾼 temporary override config를 만들어 반복 실행하면 된다. 현재 저장소에는 공식 broad multi-seed 실행 스크립트 [`../../scripts/official/run_broad_multiseed.sh`](../../scripts/official/run_broad_multiseed.sh)와 로컬 점검용 실행 스크립트 [`../../scripts/dev/run_local_sanity.sh`](../../scripts/dev/run_local_sanity.sh)가 있다.
