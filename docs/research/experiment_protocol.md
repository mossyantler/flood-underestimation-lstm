...
6. basin-count 선택은 `non-DRBC validation 성능 + static attribute distribution diagnostics + compute cost`를 함께 보고 내린다. DRBC holdout test metric은 pilot basin 수 선택에 사용하지 않는다.

따라서 Methods나 내부 결과표에서 pilot 결과를 인용할 때는 반드시 `basin-count selection pilot` 또는 `operational scaling pilot`이라고 명시하고, `main comparison result`와 섞어 쓰지 않는다.
...
prepared broad split과 split manifest는 [`../../data/CAMELSH_generic/drbc_holdout_broad/splits/`](../../data/CAMELSH_generic/drbc_holdout_broad/splits/) 아래에 둔다. 현재 프로젝트의 공식 실행 기준은 broad prepared split을 직접 읽는 broad config다. scaling pilot은 이 prepared split manifest를 실행 가능성 필터로 재사용하지만, 결과 split과 config는 `configs/pilot/` 아래에 별도로 둔다.
