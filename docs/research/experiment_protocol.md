...
현재 vendored NeuralHydrology에는 coverage/calibration built-in metric이 없으므로, `test_all_output.p` 또는 후속 export 결과를 읽는 custom evaluation script에서 계산한다. 다만 `save_all_output=True`는 `y_quantiles`뿐 아니라 recurrent state와 full sequence output까지 저장할 수 있어서, 모든 seed/model/validation epoch sweep에 일괄 적용하면 저장 공간과 I/O 비용이 커진다. 따라서 full validation-epoch DRBC test sweep은 우선 `save_all_output=False`인 built-in q50 중심 결과로 돌리고, `q90/q95/q99` coverage/calibration은 primary checkpoint와 필요한 robustness checkpoint만 별도 export 또는 lean custom evaluator로 계산한다.
