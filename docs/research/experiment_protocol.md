...
| Model 3 | physics-guided 추가 이득 검증 | `model: cudalstm` 계열 backbone + conceptual core + probabilistic head |

핵심 원칙은 `backbone은 고정하고, head와 physics 구조만 단계적으로 바꾼다`는 점이다. 그래야 Model 1 대비 Model 2의 개선을 `tail-aware output`, Model 2 대비 Model 3의 개선을 `physics-guided state/routing structure`의 효과로 해석할 수 있다.
...
공식 비교표와 논문 Methods에서는 가능한 한 같은 환경에서 Model 1과 Model 2를 돌린다. 다만 환경 차이 때문에 `batch_size`, `hidden_size`, `seq_length`, split, input 목록 같은 핵심 통제변인을 바꾸는 것은 허용하지 않는다. local safe run은 디버깅과 파이프라인 검증용이며, 논문용 공식 비교축으로 사용하지 않는다.
...
`lagged Q`는 현재 baseline 비교축에서는 쓰지 않는다. lagged observation이 들어가면 deterministic baseline이 과하게 강해지고, 이후 probabilistic head와 physics-guided core의 순수 효과를 분리하기 어렵기 때문이다.

## 3. Split 생성 규칙

세 모델은 같은 split 정의를 공유해야 한다. 같은 split에서 backbone과 head만 달라져야 비교가 성립한다.
...
3. Model 1, Model 2, Model 3는 동일한 basin file과 동일한 날짜 경계를 재사용한다.
...
prepared broad split과 split manifest는 [`../../data/CAMELSH_generic/drbc_holdout_broad/splits/`](../../data/CAMELSH_generic/drbc_holdout_broad/splits/) 아래에 둔다. broad config는 이 prepared split을 직접 읽고, local safe split은 그 prepared split을 다시 샘플링해 사용한다.
...
### 4.3 Model 3: physics-guided probabilistic hybrid

Model 3는 Model 2의 probabilistic loss 위에 conceptual core 관련 regularization을 추가한다. 현재 논문 단계에서는 아래 형태를 목표식으로 둔다.

```text
L_total = L_prob + λ_mass L_mass_balance + λ_nonneg L_nonnegativity + λ_bound L_storage_bounds
```

다만 Model 3는 아직 설계 단계이므로, 본 문서의 즉시 구현 대상은 Model 1과 Model 2로 제한한다. Model 3는 `physics regularization term`, `state variable schema`, `bounded coefficient definition`이 확정되면 별도 섹션을 확장한다.
...
Model 2와 Model 3에서는 아래 probabilistic metric을 추가 보고한다.
...
4. `coverage`
5. `calibration`
6. `Model 3 physics-guided hybrid`
