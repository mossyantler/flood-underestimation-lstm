...
현재 프로젝트의 공식 baseline 정의는 broad prepared split을 읽는 `camelsh_hourly_model1_drbc_holdout_broad`다. 즉 디펜스에서는 특정 로컬 축소 실험이 아니라, 현재 저장소에 고정된 공식 baseline 구조와 입력·출력 정의를 기준으로 설명하는 것이 맞다.
...
현재 baseline의 basin 구성은 broad prepared split 전체 `1705 / 198 / 38`이다. 따라서 디펜스에서는 engineering subset이 아니라, 공식 usability gate를 통과한 broad prepared split이 baseline의 기준 cohort라고 설명하는 것이 맞다.
...
### Q28. broad prepared split은 현재 어떻게 정의되나요?

현재 공식 broad prepared split은 `train 1705 / validation 198 / test 38`이다. 이 값은 broad split 후보에 `minimum quality gate`와 `split-level usability gate`를 적용한 결과이며, basin master checklist와 split manifest가 근거다.

### Q29. 현재 논문용 공식 run 산출물은 준비되어 있나요?

현재 저장소에는 broad config와 실행 스크립트, prepared split, split manifest가 정리돼 있다. 다만 `runs/` 산출물은 정리한 상태이므로, 공식 비교 결과표와 checkpoint는 broad run을 다시 실행해 생성해야 한다고 설명하는 것이 정확하다.

### Q30. 그러면 논문 baseline은 무엇으로 설명해야 하나요?

논문 baseline은 broad prepared split과 broad config로 정의된 `official baseline setup`으로 설명하면 된다. 즉 현재 저장소에는 실행 기준과 데이터 기준이 준비돼 있고, 최종 성능 보고는 broad run을 다시 수행한 뒤 제시한다고 말하는 편이 가장 정확하다.
...
- [`../../configs/camelsh_hourly_model1_drbc_holdout_broad.yml`](../../configs/camelsh_hourly_model1_drbc_holdout_broad.yml): 현재 공식 baseline broad config
- [`../../configs/camelsh_hourly_model2_drbc_holdout_broad.yml`](../../configs/camelsh_hourly_model2_drbc_holdout_broad.yml): 현재 공식 probabilistic broad config
