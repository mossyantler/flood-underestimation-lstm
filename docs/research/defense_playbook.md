...
`non-DRBC CAMELSH basin 중 minimum quality gate와 split-level usability gate를 통과한 broad prepared split으로 global multi-basin model을 학습하고, DRBC Delaware basin을 regional holdout evaluation region으로 두며, DRBC 내부에서는 observed-flow 기반 flood relevance screening을 통해 최종 평가 cohort를 확정한다.`
...
현재 safe baseline의 basin 구성을 그대로 발표하면 오해가 생길 수 있다. 현재 학습 run은 broad prepared split 전체 `1705/198/38`이 아니라, local hardware 제약을 반영해 `train 256 / validation 64 / test 38`로 줄인 subset이다. 따라서 이 basin 수는 scientific final split이 아니라, local end-to-end baseline을 만들기 위한 engineering subset이라고 분명히 말해야 한다.
