...
- 저장소에 남겨 둔 Model 3 follow-up 메모의 conceptual core 수식 상세

## 1. 먼저 기준선을 어떻게 잡아야 하나

우리 연구의 핵심 novelty는 `새로운 basin screening 방법`이 아니라, `같은 backbone의 deterministic LSTM과 probabilistic head를 flood-focused setting에서 비교`하는 데 있다. physics-guided hybrid 메모는 저장소에 남겨 두되, 현재 논문의 공식 비교 대상은 아니다. 이 점을 잊으면 basin selection과 screening이 연구 본체를 가리는 구조가 된다.
...
`우리는 global multi-basin training + regional holdout flood-focused evaluation setting에서 deterministic LSTM과 probabilistic LSTM을 같은 backbone에서 비교하고, output design만 바꿔도 extreme peak underestimation과 event-level flood response가 얼마나 달라지는지 평가한다.`

physics-guided hybrid는 이 비교를 바탕으로 저장소에 남겨 둔 후속 확장 방향이라고 설명하는 편이 안전하다.
...
첫 논문의 질문은 “어떤 backbone이 최고인가”가 아니라, `같은 LSTM backbone에서 output design을 바꾸면 flood underestimation이 어떻게 달라지는가`다. 그래서 backbone은 community baseline으로 가장 설명하기 쉬운 `multi-basin LSTM`으로 고정했다. 이렇게 해야 성능 차이를 backbone이 아니라 head 설계의 차이로 해석할 수 있다.
...
다섯째, `Model 2의 학습 규칙`을 문서에 더 분명히 적어야 한다. 특히 end-to-end fine-tuning이 본 실험이고, head-only는 ablation이라는 점을 명시하는 것이 좋다. Model 3 메모는 현재 논문 본 비교와 섞이지 않게 follow-up scope로 분리해 두는 편이 안전하다.
...
셋째, `Model 3는 아직 후속 확장 메모에 가깝다.` Model 1/2는 비교적 바로 실험 가능하지만, Model 3는 conceptual core와 regularization이 아직 상세 설계 단계에 가깝다. 이 상태에서 논문 타이틀과 기여를 3-model full comparison으로 너무 일찍 고정하면 위험하다.
...
현재 우리 설계는 아예 잘못된 것은 아니다. 다만 `모델 설계 논문`으로서 가장 취약한 지점은 basin 쪽이 아니라 `main story 분산`, `final observed-flow screening 미구현`, `single-seed / 날짜 근거 / fairness rule 미고정` 쪽이다. 따라서 다음 액션은 basin selection을 더 복잡하게 하는 것이 아니라, `observed-flow screening 구현`, `seed protocol 고정`, `날짜 근거 확정`, `Model 2 학습 규칙 명시`, `Model 3 follow-up 범위 분리`에 집중하는 것이 맞다.

## 관련 문서

- [`architecture.md`](architecture.md): 현재 논문의 Model 1/2 구조와 저장소에 남겨 둔 Model 3 follow-up 개념
