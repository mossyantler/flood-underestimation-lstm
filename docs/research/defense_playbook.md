...
현재 실제로 end-to-end 학습을 마친 모델은 `camelsh_hourly_model1_drbc_holdout_m3safe`다. 이 모델은 논문 최종판이라기보다, `MacBook Air M3 16GB`에서 안전하게 돌아가도록 줄인 local safe baseline이다. 따라서 디펜스에서는 이 결과를 `최종 성능`이 아니라 `현재 구현된 baseline의 구조와 동작 검증`으로 설명하는 것이 맞다.
...
현재 safe baseline의 실제 config는 아래처럼 요약할 수 있다.
...
| `hidden_size` | `64` | local safe run을 위한 축소 hidden size |
| `batch_size` | `64` | M3 16GB 환경에서 안전한 batch 크기 |
| `device` | `mps` | Apple GPU를 쓰되 `num_workers: 0`으로 메모리 폭주 방지 |
...
현재 safe baseline의 basin 구성을 그대로 발표하면 오해가 생길 수 있다. 현재 학습 run은 broad split 전체 `1720/200/38`이 아니라, local hardware 제약을 반영해 `train 256 / validation 64 / test 38`로 줄인 subset이다. 따라서 이 basin 수는 scientific final split이 아니라, local end-to-end baseline을 만들기 위한 engineering subset이라고 분명히 말해야 한다.
...
정적 커스텀 점수까지 공식 screening처럼 보이면, 심사자는 “이 논문은 flood susceptibility 논문인가, 모델 설계 논문인가?”라고 묻기 쉽다. 그래서 현재처럼 정적 점수는 내부 shortlist용으로만 낮추고, 공식 screening은 `공간 기준 -> 품질 게이트 -> observed-flow flood relevance -> hydromod split`으로 단순화하는 것이 맞다.
...
현재 broad config는 train 2000–2010, validation 2011–2013, test 2014–2016으로 잡혀 있다. 이건 실행 가능한 초기값이지만, 왜 이 기간이 hydrologically representative한지는 아직 충분히 설명되어 있지 않다.
...
이 설정은 `14일 문맥을 보고 마지막 24시간을 맞히는` 구조다. 이유는 antecedent wetness, snow/soil memory, routing memory를 어느 정도 반영하려면 단순히 1~2일 입력만으로는 부족하기 때문이다. 동시에 local safe run에서는 너무 긴 시퀀스를 쓰면 메모리 부담이 커지므로, 336시간은 `수문학적 문맥`과 `하드웨어 안정성` 사이의 타협값이라고 설명할 수 있다.
...
이 run은 `final scientific result`가 아니라 `local safe baseline`이다. broad split 전체를 macOS `MPS + num_workers>0` 조합으로 그대로 돌리면 dataset serialization 때문에 메모리가 크게 폭주했고, 실제로 machine-level freeze가 발생했다. 그래서 현재 run은 `M3 16GB` 환경에서 안정적으로 끝까지 도는 baseline을 먼저 확보하기 위해 train과 validation basin 수를 줄였다.

### Q29. validation NSE가 아직 음수인데 이 모델을 왜 유지하나요?

현재 run의 목적은 우선 `구현된 baseline이 시간 단위 CAMELSH에서 끝까지 안정적으로 학습되고, DRBC holdout 실험 파이프라인이 실제로 돈다`는 것을 확인하는 데 있다. 따라서 지금 음수 NSE는 “연구 결론”이 아니라 “현재 safe baseline의 출발점”으로 해석해야 한다. 디펜스에서는 이 결과를 정직하게 인정하고, 다음 단계가 larger split, multi-seed, Model 2 비교라고 연결하는 편이 낫다.

### Q30. 그러면 이 결과를 논문의 최종 baseline으로 써도 되나요?

아직은 어렵다. 현재 run은 `single seed`, `safe subset`, `local hardware-constrained config`라는 한계가 분명하다. 다만 이 run으로부터 `입력/출력 정의`, `loss 동작`, `regional holdout pipeline`, `safe training protocol`은 이미 검증됐으므로, 다음 baseline 실험의 실행 토대는 충분히 확보된 상태라고 말할 수 있다.
...
- [`../../configs/camelsh_hourly_model1_drbc_holdout_m3safe.yml`](../../configs/camelsh_hourly_model1_drbc_holdout_m3safe.yml): 현재 local safe baseline config
- [`../../runs/camelsh_hourly_model1_drbc_holdout_m3safe_0504_162549/output.log`](../../runs/camelsh_hourly_model1_drbc_holdout_m3safe_0504_162549/output.log): 현재 baseline 학습 로그
