...
- Model 3의 conceptual core 수식 상세

## 1. 먼저 기준선을 어떻게 잡아야 하나

우리 연구의 핵심 novelty는 `새로운 basin screening 방법`이 아니라, `deterministic LSTM -> probabilistic head -> physics-guided hybrid`라는 모델 구조 비교를 flood-focused setting에서 수행하는 데 있다. 이 점을 잊으면 basin selection과 screening이 연구 본체를 가리는 구조가 된다.
...
`non-DRBC quality-pass CAMELSH basin으로 global multi-basin model을 학습하고, DRBC Delaware basin을 regional holdout evaluation region으로 두며, DRBC 내부에서는 observed-flow 기반 flood relevance screening을 통해 최종 평가 cohort를 확정한다.`
...
둘째, `충분한 평가 basin 수`가 있기 때문이다. 현재 DRBC 기준 CAMELSH selected basin은 154개이고, quality-pass basin도 38개다. 즉 Delaware는 단일 case basin이 아니라, regional evaluation cohort를 구성할 수 있을 정도의 basin 수를 제공한다.

셋째, `상류-하류 수문 조건이 한 권역 안에서 꽤 다양하다.` 현재 quality-pass DRBC basin만 봐도 basin area는 약 10.8–4560.6 km², 경사는 약 0.18–11.57 %, forest 비율은 약 10.6–75.0 %, developed 비율은 약 4.9–80.5 % 범위를 가진다. 즉 Delaware는 하나의 권역이지만 작은 headwater부터 큰 mainstem 영향 basin까지, 비교적 다양한 flood response 조건을 포함한다.
...
### 가정 5. 연도에 값이 조금 있다고 해서 그 해를 usable year로 세지 않는다

이 가정의 이유는 관측 기간을 과대평가하지 않기 위해서다. 그래서 annual coverage가 충분한 해만 usable year로 인정한다.

### 가정 6. 정적 basin 특성은 공식 최종 screening이 아니라 보조 해석 수단이다
...
### 가정 7. final flood-relevant basin은 observed-flow metric으로 정한다
...
### 가정 8. training과 evaluation의 basin 역할은 분리한다
...
현재 발표에서는 `공식 비교 설정`과 `구현 snapshot`을 분리해서 설명하는 것이 안전하다. 공식 비교 설정은 [`experiment_protocol.md`](experiment_protocol.md)와 [`../../configs/camelsh_hourly_model1_drbc_holdout_broad.yml`](../../configs/camelsh_hourly_model1_drbc_holdout_broad.yml)을 기준으로 보고, 저장소 안에서 end-to-end 연결을 확인한 구현 snapshot 예시는 `camelsh_hourly_model1_drbc_holdout_m3safe`로 둔다. 따라서 디펜스에서는 이 섹션을 `최종 성능 보고`가 아니라 `현재 구현된 baseline의 구조와 동작 검증 예시`로 설명하는 것이 맞다.
...
현재 구현 snapshot의 config 예시는 아래처럼 요약할 수 있다.
...
| `hidden_size` | `64` | 구현 snapshot 예시에서 사용한 hidden size |
| `batch_size` | `64` | 구현 snapshot 예시에서 사용한 batch size |
| `device` | `mps` | 구현 snapshot 예시에서는 `mps`와 `num_workers: 0`을 사용한다 |
...
현재 구현 snapshot의 basin 구성을 그대로 발표하면 오해가 생길 수 있다. 이 예시는 broad split 전체 `1720/200/38`이 아니라, `train 256 / validation 64 / test 38`로 줄인 subset을 사용한다. 따라서 이 basin 수는 논문 본문의 최종 비교 기준이 아니라, end-to-end 구현 검증과 초기 비교 준비를 위한 reduced implementation subset이라고 분명히 말해야 한다. 최종 비교 설정과 split 규칙은 이 문서가 아니라 [`experiment_protocol.md`](experiment_protocol.md)와 broad config를 기준으로 본다.
...
정적 커스텀 점수까지 공식 screening처럼 보이면, 심사자는 “이 논문은 flood susceptibility 논문인가, 모델 설계 논문인가?”라고 묻기 쉽다. 그래서 현재처럼 정적 점수는 내부 shortlist용으로만 낮추고, 공식 screening logic 자체는 workflow canonical 문서에 두되 발표에서는 `공간 기준 -> 품질 게이트 -> observed-flow flood relevance -> hydromod split`의 흐름만 요약하는 것이 맞다.

### 3.3 final observed-flow screening이 아직 구현 전이다

이건 지금 가장 큰 실제 취약점이다. 지금은 DRBC 154 basin 중 quality-pass 38 basin까지는 정리됐지만, `annual peaks`, `Q99 frequency`, `RBI`, `event runoff coefficient`를 계산한 final event-response table이 아직 없다. 따라서 “이 basin들이 flood-relevant하다”는 주장은 아직 provisional 단계다.

즉 현재는 `평가 가능한 basin`까지 정리된 상태이지, `flood-focused 최종 평가 cohort`가 확정된 상태는 아니다.

### 3.4 비교 실험의 공정성 규칙이 아직 약하다

현재 config는 잡혀 있지만, `몇 개 seed로 돌릴지`, `Model 1과 Model 2를 완전히 같은 hyperparameter budget으로 비교할지`, `Model 3는 언제 본 실험에 넣을지`, `head-only ablation과 full fine-tuning을 어떻게 나눌지`가 아직 명확히 잠기지 않았다. 모델 비교 논문에서는 이 부분이 basin selection보다 더 자주 공격받는다.
...
현재 답변은 “공식 screening 기준은 workflow canonical 문서에 고정되어 있고, 발표에서는 그 흐름만 high-level로 요약한다”로 가야 한다. 즉 디펜스에서는 basin selection의 흐름을

1. DRBC와 outlet/overlap으로 공간 선택,
2. usable years / estimated-flow / boundary confidence 품질 게이트,
3. observed-flow 기반 flood relevance,
4. hydromod broad / natural split
...
### Q15. Model 1, Model 2, Model 3 비교는 정말 공정한가요?
...
현재 저장소의 example config들은 `seed: 111`로 시작하므로 이 질문은 피하기 어렵다. 다만 이것을 최종 비교 규칙처럼 방어하기보다, 현재 shipped config의 기본값이라고 설명하는 편이 맞다. 모델 비교 논문에서 single seed는 취약하므로 최소 `3개`, 가능하면 `5개` seed를 두고, basin-aggregate metric에 대해 평균과 분산 또는 bootstrap confidence interval을 보고하는 것이 좋다.

이건 지금 설계에서 가장 빨리 고쳐야 할 부분 중 하나다.

### Q17. 왜 이 날짜 구간(2000–2016)을 썼나요?

현재 broad example config는 train 2000–2010, validation 2011–2013, test 2014–2016으로 잡혀 있다. 이건 실행 가능한 초기값이지만, 왜 이 기간이 hydrologically representative한지는 아직 충분히 설명되어 있지 않다.
...
이 질문에는 먼저 인정하는 것이 좋다. large-sample neural hydrology의 모델 설계 논문들은 대체로 benchmark basin set을 유지하고, screening을 복잡하게 가져가지 않는다. 우리도 그 방향으로 수정해서, 현재는 정적 heuristic을 공식 screening에서 내리고 observed-flow 중심의 단순 screening만 남겼다고 답하면 된다.
...
`우리는 global multi-basin training + regional holdout flood-focused evaluation setting에서 deterministic LSTM, probabilistic LSTM, physics-guided probabilistic hybrid를 같은 backbone 계열에서 비교하고, 특히 extreme peak underestimation과 event-level flood response에서 어떤 구조가 실제로 이득을 주는지 평가한다.`
...
이 설정은 현재 baseline config에서 채택한 실행 기본값이다. 디펜스에서는 이 숫자를 보편 법칙처럼 방어하기보다, **"수문학적 선행조건을 어느 정도 포착하면서도 시간 단위 LSTM을 안정적으로 학습시키기 위한 실무적 절충"**으로 설명하는 편이 안전하다.

- `seq_length=336`은 14일 문맥을 보며 antecedent wetness, snow/soil memory, routing memory를 어느 정도 반영하려는 설정이다.
- `predict_last_n=24`는 입력 전체를 다 통과한 뒤 마지막 24시간 구간에서 supervision과 evaluation을 수행해, `다음 24시간의 hourly Streamflow sequence`를 예측하는 현재 baseline 구조를 만든다.
- 중요한 점은 이 값이 논문 전체의 영구 규칙이라는 주장이 아니라, 현재 baseline 비교에서 일관되게 사용하는 설정이라는 점이다.

꼬리 질문으로 "14일이면 snowpack이나 깊은 baseflow를 보기엔 짧지 않은가"가 나오면, 맞다고 인정한 뒤 본 비교의 초점이 `장기 물수지 재현`이 아니라 `단기 extreme flood response 비교`에 있다고 선을 긋는 것이 좋다.

### Q26. 왜 LSTM을 backbone으로 썼나요?

첫 논문의 질문은 “어떤 backbone이 최고인가”가 아니라, `deterministic -> probabilistic -> physics-guided`로 갈 때 flood underestimation이 어떻게 바뀌는가다. 그래서 backbone은 community baseline으로 가장 설명하기 쉬운 `multi-basin LSTM`으로 고정했다. 이렇게 해야 성능 차이를 head와 physics 구조의 차이로 해석할 수 있다.

### Q27. 왜 `lagged Q`를 넣지 않았나요?

`lagged Q`를 넣으면 short-horizon 예측 성능은 좋아질 수 있지만, baseline이 지나치게 강해져서 probabilistic head나 physics-guided core의 순수 효과를 분리하기 어려워진다. 현재 baseline은 forcing와 basin attributes만으로 학습한 뒤, 그 위에서 head 구조를 바꿔도 tail bias가 줄어드는지 보려는 설계다.

### Q28. 왜 broad split 전체가 아니라 `256/64/38` basin만 썼나요?

이 질문에는 `공식 비교 설정`과 `구현 snapshot`을 분리해서 답하는 것이 정확하다. broad split 전체 `1720/200/38`은 논문용 비교 설정이고, `256/64/38`은 현재 저장소 안에서 end-to-end 연결을 확인한 reduced implementation subset이다. 따라서 디펜스에서는 `256/64/38`을 최종 과학적 결과가 아니라 구현 검증 예시라고 설명하는 것이 맞다.

### Q29. validation NSE가 아직 음수인데 이 모델을 왜 유지하나요?

현재 reduced implementation snapshot의 목적은 우선 `구현된 baseline이 시간 단위 CAMELSH에서 끝까지 학습되고, DRBC holdout 실험 파이프라인이 실제로 돈다`는 것을 확인하는 데 있다. 따라서 여기서 나온 음수 NSE는 “연구 결론”이 아니라 “현재 구현 상태에서 확인된 출발점”으로 해석해야 한다. 디펜스에서는 이 결과를 정직하게 인정하고, 다음 단계가 broad split, multi-seed, Model 2 비교라고 연결하는 편이 낫다.

### Q30. 그러면 이 결과를 논문의 최종 baseline으로 써도 되나요?

아직은 어렵다. 현재 저장소에서 확인된 구현 snapshot은 `single seed`이고, basin 구성도 broad split 전체가 아니라 `reduced subset`이라는 한계가 분명하다. 다만 이 예시로부터 `입력/출력 정의`, `loss 동작`, `regional holdout pipeline`, `현재 학습 실행 절차`는 이미 검증됐으므로, broad split 기반 비교 실험의 실행 토대는 확보된 상태라고 말할 수 있다.
...
다섯째, `Model 2와 Model 3의 학습 규칙`을 문서에 더 분명히 적어야 한다. 특히 end-to-end fine-tuning이 본 실험이고, head-only는 ablation이라는 점을 명시하는 것이 좋다.
...
셋째, `Model 3가 아직 설계 문서 대비 구현 성숙도가 낮다.` Model 1/2는 비교적 바로 실험 가능하지만, Model 3는 conceptual core와 regularization이 아직 상세 설계 단계에 가깝다. 이 상태에서 논문 타이틀과 기여를 3-model full comparison으로 너무 일찍 고정하면 위험하다.

넷째, `flood relevance evidence가 아직 event level에서 닫히지 않았다.` 지금은 quality-pass basin까지는 확정됐지만, final flood-relevant cohort는 아직 아니다.
...
현재 우리 설계는 아예 잘못된 것은 아니다. 다만 `모델 설계 논문`으로서 가장 취약한 지점은 basin 쪽이 아니라 `main story 분산`, `final observed-flow screening 미구현`, `single-seed / 날짜 근거 / fairness rule 미고정` 쪽이다. 따라서 다음 액션은 basin selection을 더 복잡하게 하는 것이 아니라, `observed-flow screening 구현`, `seed protocol 고정`, `날짜 근거 확정`, `Model 2/3 학습 규칙 명시`에 집중하는 것이 맞다.

## 관련 문서

- [`architecture.md`](architecture.md): 세 모델의 backbone, head, conceptual core 구조
- [`experiment_protocol.md`](experiment_protocol.md): split, loss, metric, config key 규칙
- [`../../configs/camelsh_hourly_model1_drbc_holdout_broad.yml`](../../configs/camelsh_hourly_model1_drbc_holdout_broad.yml): 현재 broad comparison config 예시
- [`../../configs/camelsh_hourly_model1_drbc_holdout_m3safe.yml`](../../configs/camelsh_hourly_model1_drbc_holdout_m3safe.yml): reduced-basin implementation snapshot config 예시
