...
즉 이 문서는 `개념 설명`이 목적이다. 실제 key와 metric 규칙은 [`../research/experiment_protocol.md`](../research/experiment_protocol.md), 세 모델 전체 구조는 [`../research/architecture.md`](../research/architecture.md)에서 다룬다.
...
이 프로젝트에서 probabilistic head를 먼저 넣으려는 이유도 여기 있다. physics-guided 구조를 넣기 전에, 우선 `출력 설계만 바꿔도 peak bias가 줄어드는지`를 확인하려는 것이다.
...
아니다. 이 단계는 어디까지나 `output design`을 바꾸는 것이다. physics-guided 구조는 다음 단계인 Model 3의 주제다.
...
- 세 모델의 전체 구조는 [`../research/architecture.md`](../research/architecture.md)에서 다룬다.
