...
| calibration | 모델이 말한 확률 수준과 실제 빈도가 잘 맞는지 보는 진단이다. 우리말로는 보정이라고 부른다. 예측을 신뢰할 수 있는지 볼 때 sharpness와 짝으로 본다. |
| sharpness | 예측 분포가 얼마나 좁고 단정적인지를 나타낸다. 우리말로는 예리함이다. 폭이 좁을수록 sharp하지만, calibration이 나쁘면 좁기만 하고 빗나갈 수 있으므로 둘을 함께 본다. |
...
## 상위 quantile 이득·비용 측정 용어

Model 2의 상위 예측선(`q90 / q95 / q99`)이 홍수 첨두를 얼마나 잘 잡는지(이득)와 그 대가로 얼마나 헛경보를 내는지(비용)를 event 단위로 측정할 때 쓰는 용어다. 앞 세 개(α, β, δ)는 이득 쪽, 뒤 두 개(FAR, over-prediction magnitude)는 비용 쪽이다.

| 용어 | 쉬운 설명 |
| --- | --- |
| α (per-event peak deficit) | 사건별 첨두 부족분이다. 실제 첨두를 모델 예측선이 얼마나 못 따라갔는지를 비율로 본다. 값이 0에 가까울수록 첨두 시각에서 예측이 실제 높이를 거의 따라잡았다는 뜻이다. |
| β (±6h window capture) | 첨두 시점 앞뒤 6시간 창 안에서 모델이 큰 값을 잡았는지를 본다. 1에 가까우면 그 창 안의 실제 최댓값만큼 예측했다는 뜻이고, 1보다 크면 그보다도 높게 예측했다는 뜻이다. |
| δ (threshold recall) | 큰 물 기준선인 `Q99`를 넘는 순간을 모델이 얼마나 잡아냈는지를 보는 재현율이다. 우리말로는 기준 초과 재현율이다. 실제로 기준을 넘은 시각 중 모델도 넘는다고 본 비율로, 높을수록 극한 시점을 덜 놓친다. |
| FAR (false alarm rate) | 헛경보 비율이다. 실제로는 `Q99` 기준을 안 넘었는데 모델이 넘는다고 잘못 본 시각의 비율이다. 상위 quantile을 올릴수록 δ(이득)와 함께 이 값(비용)도 커진다. |
| over-prediction magnitude | 과대예측의 크기다. 모델이 실제 관측보다 높게 낸 경우, 그 초과분이 평균적으로 얼마나 큰지를 본다. 첨두를 잡으려고 상위 quantile을 올리면 이 값도 같이 커진다. |
