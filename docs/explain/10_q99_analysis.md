...
이 문서는 "q99 분석"이 무엇인지 비전공 대학생 기준으로 처음부터 끝까지 풀어 쓴다. 수문학이나 기계학습 배경을 전혀 전제하지 않는다. 어려운 용어가 나올 때마다 바로 옆에서 쉽게 설명한다. 동시에 "이 수치가 코드 어디에서 어떻게 계산되는가"를 보고서식 문장으로 본문에 녹여 두었다. 직접 코드를 들여다볼 사람을 위한 안내이자, "근거 없이 지어낸 숫자가 아니다"라는 증빙이기도 하다.
...
분석 대상: DRBC(델라웨어강 유역 위원회) 지역 관측 가능한 85개 유역, 학습 seed(난수 씨앗, 모델 초기화 값) 세 벌 `111 / 222 / 444`, 테스트 기간 `2014–2016`. 이 세 가지 고정 조건은 분석 스크립트들이 공유하는 설정 파일 `scripts/_lib/expanded_drbc.py`에 상수로 박혀 있다. 학습 기간(설정값 `TRAIN_PERIOD`)은 `2000-01-01 ~ 2010-12-31`, 테스트 기간(설정값 `TEST_PERIOD`)은 `2014-01-01 ~ 2016-12-31`, seed 목록(설정값 `SEEDS`)은 `(111, 222, 444)`로 명시되어 있다. 분석 스크립트가 이 값을 일일이 새로 정하지 않고 한 파일에서 가져다 쓰기 때문에, 모든 산출물이 같은 기간·같은 seed 위에서 계산됨이 보장된다.
...
이 빨간 선이 코드에서 실제로 어떻게 그어지는지는 분석의 첫 단추라 정확히 짚고 넘어간다. 큰 물 사건을 추려 내는 스크립트 `scripts/model/expanded_drbc/build_q99_events.py` 안에는 `basin_q99_from_netcdf`라는 함수가 있는데, 이 함수는 유역별 원자료 파일(`data/CAMELSH_generic/drbc_expanded_observed_test/time_series/<유역>.nc`)에서 변수명 `Streamflow`(관측 유량)를 학습 기간(설정값 `TRAIN_PERIOD`, 즉 2000–2010)만큼 잘라 낸 뒤, 그 값들의 99분위수(설정값 `HIGH_FLOW_PERCENTILE = 0.99`)를 계산한다. 이렇게 나온 한 유역의 한 숫자가 그 유역의 대문자 Q99 기준선이다. 핵심은 이 계산에 **학습 기간 관측값만** 쓴다는 점이다. 테스트 기간(2014–2016)의 실제 유량은 기준선을 정하는 데 끼어들지 않으므로, "정답을 미리 엿보고 기준을 맞췄다"는 관측값 누수(테스트 정답이 분석에 새어 드는 일)가 생기지 않는다.
...
두 모델의 출력은 분석 스크립트가 읽는 공통 입력 파일에 나란히 담겨 있다. seed별로 한 파일씩, `output/model_analysis/primary/metrics/data/required_series/seed{111,222,444}/required_series.csv` 형태로 저장되며, 한 행마다 유역(열 이름 `basin`), 시각(열 이름 `datetime`), 관측 유량(열 이름 `obs`), Model 1 예측(열 이름 `model1`), 그리고 Model 2의 네 예측선(열 이름 `q50` `q90` `q95` `q99`)이 들어 있다. 뒤에 나오는 모든 성능 수치는 결국 이 한 표의 `obs`와 예측 열들을 비교해서 나온다.
...
"큰 물 기준선을 넘는 사건만 모은다"는 첫 단계는 앞서 본 `build_q99_events.py`가 담당한다. 이 스크립트의 `find_events_for_basin` 함수는 테스트 기간 관측값(`obs`) 중 그 유역의 대문자 Q99를 넘는 시각만 골라낸 뒤, 시간상 가까이 붙은 시각들을 하나의 "사건"으로 묶는다. 끊어진 두 시각의 간격이 설정값 `EVENT_MERGE_GAP_HOURS`(12시간) 이내면 같은 사건으로, 그보다 멀면 별개 사건으로 본다. 한 사건의 첨두 시각은 그 묶음 안에서 관측 유량이 가장 높은 시점으로 정한다. 이렇게 만들어진 사건 목록이 성능 갈래(3절)의 분석 단위가 된다.
...
세 잣대를 한눈에 보는 참조 카드부터 둔다. 상세 풀이는 카드 아래 각 H3 절에서 이어진다.

| 심볼 | 변수명 | 범위 | 최적화 방향 |
| --- | --- | --- | --- |
| α | `rq2_alpha_event_peak_deficit` | 0~1 | 작을수록 좋음 ↓ |
| β | `rq2_beta_window_capture` | 0~(1 초과 가능) | 1에 가까울수록 좋음, 너무 크면 과대 |
| δ | `rq2_delta_threshold_recall` | 0~1 | 클수록 좋음 ↑ |

### 3-1. α — 첨두 시각에서 모자란 정도 (event peak under-deficit)
...
이 값은 스크립트 `scripts/model/expanded_drbc/compute_rq2_alpha_peak_deficit.py`의 `compute_alpha` 함수가 계산한다. 각 사건의 첨두 시각(`peak_time`)에서 관측 첨두값과 각 예측선의 값을 꺼내, `max(0, (관측 첨두 − 예측)) / 관측 첨두` 식으로 모자란 비율을 구한다. 식 앞에 `max(0, …)`가 붙어 있어서, 예측이 관측보다 높게 잡힌 경우(넘겨 잡음)는 음수로 깎이지 않고 0으로 처리된다. 즉 α는 "모자란 쪽만" 센다.
...
### 3-2. β — 첨두 부근 6시간 창을 덮는 비율 (±6h window capture)
...
여기서 "6시간 창"은 임의로 잡은 숫자가 아니라 설정값 `EVENT_WINDOW_HOURS`(6)에서 온다. 사건을 만든 `build_q99_events.py`가 첨두 시각을 중심으로 앞뒤 6시간씩 창을 잘라 두고(창이 테스트 기간 끝을 넘어가면 잘라 내며 표시), 스크립트 `scripts/model/expanded_drbc/compute_rq2_beta_window_capture.py`가 그 창 안에서 예측 최댓값과 관측 최댓값의 비율을 구한다.
...
### 3-3. δ — 큰 물 시각을 덮는 비율 (Q99 threshold recall)
...
이 값은 스크립트 `scripts/model/expanded_drbc/compute_rq2_delta_threshold_recall.py`의 `compute_recall_for_seed` 함수가 계산한다. 이 함수는 먼저 관측값이 그 유역의 대문자 Q99를 넘는 시각(`obs >= q99_train_value`)만 추린 다음, 그중 예측선이 관측을 덮은 시각(`예측 ≥ obs`)의 개수를 전체 큰 물 시각 수로 나눈다. α·β가 "사건 첨두 한 점"을 보는 것과 달리, δ는 Q99를 넘은 **모든 시각을 한데 모아(pooled)** 비율을 내므로, 사건 안 어느 시점이든 빠짐없이 따진다는 차이가 있다.
...
NOAA(미국 해양대기청, 공식 홍수 기록 기관)가 실제 홍수로 확정한 사건만 따로 모아 다시 확인했다. 테스트 기간(2014–2016)에 DRBC 85개 유역과 겹치는 21개 유역, 65개 사건이 대상이다. α·β 스크립트는 같은 코드로 사건 목록만 바꿔(대문자 Q99 초과 사건 대신 NOAA 확정 홍수 사건) 다시 돌린 것이라, 측정 방식 자체는 동일하다.
...
원인 분석은 세 방향에서 파고든다. 출처 보고서(`docs/report/q99_causes/q99_analysis_report.md`)는 이 셋을 각각 Analysis B(유역 특성), Analysis 3(기상 조건), Analysis A(모델 내부)로 부른다.

1. **어떤 유역에서** 어긋나는가 (유역 특성)
2. **어떤 비에서** 어긋나는가 (이벤트 기상 조건)
3. **모델이 무엇을 보고** q99를 정하는가 (모델 내부 기여도)

### 4-0. 먼저 — 두 방향의 어긋남과 세 가지 오차 지표
...
#### 중앙 상대 편향 — 예측이 평균적으로 어느 쪽으로 치우치나

큰 물 시각에서 `(예측 − 관측) ÷ 관측`을 모든 시각에 대해 구한 뒤 그 중앙값을 본 것이다. 양수면 평균적으로 부풀려 잡음(과대), 음수면 낮게 잡음(과소)이다. 소스 문서 기준 q99의 중앙값은 +0.115(과대 방향), Model 1은 −0.466(과소 방향)이다.

#### 과소추정 비율 — 낮게 잡은 시각이 얼마나 잦은가

큰 물 시각 중 예측이 관측보다 낮았던 시각의 비율이다. q99는 이 값을 약 84%(Model 1)에서 약 41%(중앙값 0.410)로 낮췄다. "덜 놓친다"는 q99의 강점이 여기서 수치로 드러난다.

#### 편향 차이 — q99가 Model 1보다 얼마나 나아졌나
...
CAPE(대기 중 잠재적 에너지 지수, Convective Available Potential Energy)는 대류 불안정도를 나타낸다. 뇌우나 국지성 집중호우가 얼마나 강할지 가늠하는 기상 지표다. 이것이 Q99 오차와 무관하게 나왔다(3-seed stable 조건을 통과하지 못했다). 즉 CAPE가 높다고 해서 모델이 더 잘 맞거나 더 못 맞는 것이 아니었다. 모델이 이 신호를 제대로 활용하지 못하기 때문일 가능성이 있다.
...
> **수치 출처:** 성능 갈래(3절) 수치는 `docs/experiment/analysis/model/02_upper_quantile_peak_under.md`, 원인 갈래(4절) 수치는 `docs/report/q99_causes/q99_analysis_report.md`에 적힌 값만 인용했다. 계산 방식·설정값·함수명은 `scripts/_lib/expanded_drbc.py`와 `scripts/model/expanded_drbc/`의 `build_q99_events.py` · `compute_rq2_alpha_peak_deficit.py` · `compute_rq2_beta_window_capture.py` · `compute_rq2_delta_threshold_recall.py`를 직접 확인해 적었다. 없는 부분은 정성적으로만 서술했으며, 확인되지 않은 인과를 단정하지 않았다.
</content>
</invoke>
