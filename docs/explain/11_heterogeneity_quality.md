...
이 세 분석은 각각 별도의 계산 스크립트가 만든 수치에 기반한다. 유역별 차이(RQ-4a)는 `scripts/model/expanded_drbc/compute_rq4a_nse_tier_stratify.py`가, 홍수 유형별 차이(RQ-4b)는 같은 폴더의 `compute_rq4b_event_type_stratify.py`가, 예측선 품질 점검(RQ-5)은 `scripts/model/hydrograph/analyze_expanded_drbc_probabilistic_diagnostics.py`가 담당한다. 본문에서 어떤 수치가 어느 스크립트의 어느 계산에서 나왔는지를 그때그때 짚어 둔다. 모두 산출물 폴더 `output/model_analysis/primary/metrics`(RQ-4a·4b)와 `output/model_analysis/expanded_drbc_test/probabilistic_diagnostics`(RQ-5)에 결과를 쓴다.
...
두 모델의 구조 차이는 8장(`docs/explain/02_model_structure.md`)에서 자세히 다룬다. 여기서 중요한 점만 다시 짚으면, 두 모델은 입력 자료와 LSTM 몸통을 똑같이 쓰고 마지막 출력층(설정 항목 `head`)만 다르다. 그래서 성능 차이가 "출력 방식만 바꾼 효과"인지 깔끔하게 분리된다.
...
이 묶음 나누기는 손으로 경계값을 정한 것이 아니라 코드가 자동으로 균등 3등분한다. 계산 스크립트 `compute_rq4a_nse_tier_stratify.py`는 먼저 유역별 Model 1 NSE를 모은 표 `rq1_central_metrics_seed_median.csv`에서 NSE 값(변수명 `m1_nse_seed_median`)을 읽는다. 이 값은 seed 세 개(`111 / 222 / 444`)의 NSE를 유역마다 중앙값으로 합친 것이다. 그런 다음 pandas의 `qcut`이라는 함수에 "세 등분"(인자 `q=3`)을 시키면, 전체 유역을 NSE 크기 순으로 줄 세운 뒤 아래 1/3을 `bottom`, 가운데 1/3을 `mid`, 위 1/3을 `top`으로 자동 표시한다. 즉 묶음의 경계선은 "85개 유역 NSE 분포의 33퍼센타일·67퍼센타일"이며, 사람이 임의로 0.5 같은 숫자를 박아 넣은 것이 아니다.
...
**이 연구의 설계:** 나누는 기준(Model 1 NSE)과 평가하는 지표(첨두 놓침·잡은 비율·거짓경보)는 **개념적으로 다른 측면**이다. NSE는 전체 기간 유량 곡선의 형태 유사도이고, 첨두 놓침은 극단 사건에서의 최댓값 과소추정이다. 이 둘은 연관이 약하므로 강한 순환은 아니다. 코드 차원에서도 이 분리가 지켜진다. `compute_rq4a_nse_tier_stratify.py`는 묶음을 나눌 때는 NSE 표 하나만 읽고, 평가 지표는 완전히 다른 표 다섯 개(α는 `rq2_alpha_event_peak_deficit_q99.csv`, β는 `rq2_beta_window_capture_q99.csv`, 재현율은 `rq2_delta_threshold_recall_per_basin_seed.csv`, 거짓경보·초과예측은 `rq3_*` 표)에서 따로 읽어 합친다. 다만 완전히 독립적이지도 않으니, 아래 숫자는 "확정 인과"가 아닌 **패턴 경향**으로 읽는다.
...
| 심볼 | 변수명 | 값 범위 | 좋은 방향 |
| --- | --- | --- | --- |
| α | `alpha_basin_median` (peak deficit) | 0 ~ 1 | 0에 가까울수록 좋음 |
| β | `beta_basin_median` (window capture) | 0 이상 | 1 이상이면 첨두 주변을 충분히 덮음 |
| recall | `delta_basin_median` (threshold recall) | 0 ~ 1 | 1에 가까울수록 좋음 |
| FAR | `far_basin_median` | 0 ~ 1 | 0에 가까울수록 좋음 |
| over-pred | `over_pred_basin_median` | 0 이상 (m³/s) | 작을수록 부담 적음 |

#### α (알파, 첨두 놓침)

예측 첨두에 비해 관측 첨두가 얼마나 더 높았는지를 0~1 사이 비율로 나타낸 값이다. 0이면 예측이 관측 첨두를 정확히 따라잡았다는 뜻이고, 1에 가까울수록 관측이 예측보다 훨씬 높아 크게 놓쳤다는 뜻이다.

#### β (베타, 창 포착률)

홍수 첨두 시각의 앞뒤 ±6시간 안에서 예측선이 관측을 얼마나 덮었는지다. 1을 넘으면 첨두 위아래 시간대까지 충분히 감쌌다는 뜻이다.

#### recall (재현율)

관측 유량이 매우 컸던 시각들 중에서 예측선이 실제로 그 큰 값을 맞게 잡아낸 비율이다. 1에 가까울수록 큰 유량 순간을 거의 다 잡았다는 뜻이다.

#### FAR (거짓경보율)

실제로는 크지 않았던 시각인데 예측선이 "크다"고 경보를 울린 비율이다. 0이면 헛경보가 없다는 뜻이고, 클수록 안 왔는데 울린 경우가 많다는 뜻이다.
...
유형 분류는 사람이 사건마다 손으로 라벨을 단 것이 아니라, NOAA 기록의 설명 문장(필드명 `noaa_annotation`)을 정규식(글자 패턴 규칙)으로 읽어 자동 분류한다. 이 규칙은 공용 helper `scripts/_lib/expanded_drbc`의 정규식 묶음에 들어 있다. 한 사건에 여러 단어가 겹쳐 보일 때는 "더 구체적인 유형이 이긴다"는 우선순위(돌발홍수 > 해안홍수 > 일반홍수 > 기타)로 정한다. 예를 들어 "Flash Flood"라는 표현은 "Flood"보다 구체적이므로 돌발홍수로 분류된다. 단 "Flood Watch", "Flood Advisory"처럼 경보·주의보 안내 문구는 실제 발생 사건이 아니므로 제외하도록 규칙이 짜여 있다.
...
65개 사건은 Flash Flood 8개, Flood(일반) 32개, NOAA 기록 없는 NWS 기준 사건 25개로 나뉜다. 이 "test 기간 안" 추리기는 스크립트 `compute_rq4b_event_type_stratify.py`가 사건 표를 읽은 뒤 첨두 시각(변수명 `peak_time`)이 2014-01-01부터 2016-12-31 23시 사이인 행만 남기고, 동시에 "85개 유역에 속함" 표시(변수명 `in_expanded_85`)가 참인 행만 골라 만든 결과다. Coastal Flood는 test 기간 기준 해당 사건이 너무 적어 표에서 제외했다. 코드에서도 사건 수가 5개 미만인 유형(상수 `MIN_GROUP_EVENTS = 5`)은 자동으로 "Other"로 묶어 별도 표시를 못 하게 막는다.
...
이 점검의 계산은 모두 `analyze_expanded_drbc_probabilistic_diagnostics.py` 하나가 담당한다. 이 스크립트는 seed별 예측·관측이 모인 시계열 표(`required_series`)를 읽되, 관측값이 비어 있는(NaN) 시각은 먼저 버린다(함수 `_read_expanded_series`). 그래서 아래 모든 비율의 분모는 "관측이 실제로 존재하는 시각"만 센다.

### calibration을 확인하는 방법

방법은 간단하다. test 기간 전체 시각(약 200만 시간·유역 조합)에서, 관측 유량이 각 예측선 아래로 들어온 비율을 직접 센다. 코드 안에서도 이 계산은 글자 그대로 "관측이 예측선 이하인 시각의 비율"을 구하는 한 줄(개념식 `(obs <= 예측선).mean()`)로 이루어진다.
...
이렇게 실제 비율을 재는 것이 **실제 포함율(empirical coverage)** 확인이다.

### 결과: 전체 기간 calibration 점검

먼저 간결 참조 카드로 정리한 뒤, 표 아래에서 자세히 푼다.

| 심볼 | 변수명 | 이름값(이론) | 실제 포함율 |
| --- | --- | --- | --- |
| q50 | `median_empirical_coverage` (τ=0.50) | 0.50 | 0.339 |
| q90 | `median_empirical_coverage` (τ=0.90) | 0.90 | 0.506 |
| q95 | `median_empirical_coverage` (τ=0.95) | 0.95 | 0.638 |
| q99 | `median_empirical_coverage` (τ=0.99) | 0.99 | 0.787 |
...
| 심볼 | 변수명 | 범위 | 의미 |
| --- | --- | --- | --- |
| q50 | tail hit-rate (τ=0.50) | 0 ~ 1 | 큰 유량 시각 중 덮은 비율 |
| q90 | tail hit-rate (τ=0.90) | 0 ~ 1 | 〃 |
| q95 | tail hit-rate (τ=0.95) | 0 ~ 1 | 〃 |
| q99 | tail hit-rate (τ=0.99) | 0 ~ 1 | 〃 |

#### 결과 수치와 해석

큰 유량 시각(Q99 초과)에서의 적중률은 q50 0.218, q90 0.371, q95 0.424, q99 **0.563** 이다. `q99` 선은 관측 유량이 매우 컸던 시각의 **56%** 를 덮고, 나머지 44%는 여전히 놓친다.
...
여기서 한 가지를 분명히 해야 한다. 이 baseline은 **훈련 기간 자료만** 쓴다. 스크립트의 함수 `_load_train_climatology`는 각 유역 NetCDF 파일에서 2000-2010 구간(상수 `TRAIN_PERIOD_START` ~ `TRAIN_PERIOD_END`) 관측 유량만 떼어 quantile을 계산하고, 그 창을 벗어난 자료가 섞이지 않았는지 코드가 직접 점검(assert)한다. test 기간 자료가 baseline에 새어 들어가면 비교가 불공정해지기 때문이다. 이것이 "관측값 누수 방지"다.

이 baseline보다 Model 2가 얼마나 나은지를 **기술 점수(skill score)** 로 잰다. 계산식은 "1 - (모델 오차 / 과거평균 오차)"이며(여기서 오차는 quantile 학습용 pinball loss), 양수면 baseline보다 낫고 음수면 baseline보다 못하다.

| 심볼 | 변수명 | 범위 | 좋은 방향 |
| --- | --- | --- | --- |
| q50 | `pinball_skill_score` (τ=0.50) | -∞ ~ 1 | 양수면 baseline보다 좋음 |
| q90 | `pinball_skill_score` (τ=0.90) | -∞ ~ 1 | 〃 |
| q95 | `pinball_skill_score` (τ=0.95) | -∞ ~ 1 | 〃 |
| q99 | `pinball_skill_score` (τ=0.99) | -∞ ~ 1 | 〃 |

#### 결과 수치

q50 +0.104, q90 **+0.217**, q95 +0.132, q99 **-0.271** 이다. `q50 / q90 / q95`는 모두 양수다. Model 2가 단순 과거 평균보다 더 좋은 예측을 한다는 뜻이다. 그런데 `q99`는 -0.271이다. **과거 평균보다 오히려 나쁘다.**
