...
이 문서는 처리 단계를 순서대로 따라가는 대신 데이터 자체를 구성 축으로 설명한다. 즉 데이터셋의 정의와 원자료 출처, 섞여 있는 자료의 종류, 모델의 입력과 출력, 그리고 유역의 train·validation·test 분리 기준을 차례로 정리한다. 실제 처리를 수행하는 스크립트·설정 파일·데이터 위치는 본문 산문에 끼워 넣지 않고 각 절 끝의 표로 따로 모은다.
...
자료가 이 세 종류로 나뉘어 있다는 점이 뒤의 split·test 확장 설계에 직접 영향을 준다. 어떤 유역은 forcing과 관측 유량이 한 아카이브(`timeseries.7z`)에 같이 들어 있지만, 어떤 유역은 forcing(`timeseries_nonobs.7z`)과 관측 유량(`Hourly2.zip`)이 서로 다른 원본에 흩어져 있어 둘을 따로 가져와 짝지어야 한다.
...
dynamic forcing은 시간에 따라 변하며 유역을 외부에서 구동하는 입력 조건으로, 이 연구에서는 기상 변수가 이에 해당한다. 모델은 현재 11개 forcing 변수를 입력으로 받는다. 홍수 첨두는 유역으로 들어오는 물의 양과 유역의 반응 방식이 함께 결정하므로, forcing 변수는 강수 자체뿐 아니라 강수의 성격과 강수 전후의 에너지·수분 조건까지 포괄한다.
...
첫째, 강수의 절대량은 `Rainf`가 담는다. 유역으로 들어오는 직접적인 물 공급이며, 짧은 시간에 솟구치는 유량의 꼭대기인 홍수 첨두를 만드는 핵심 forcing이다.
...
> forcing 변수 목록은 Model 2 설정 파일[^cfg-m2]의 `dynamic_inputs` 항목에서 정의한다.
...
> 정적 속성 목록은 Model 2 설정 파일[^cfg-m2]의 `static_attributes` 항목에서 정의한다.

## 5. 출력: Streamflow

모델이 맞히려는 정답(target)은 하천의 시간별 유량인 `Streamflow` 하나다. 물리적으로 유량은 음수가 될 수 없으므로, target을 0 아래로 내려가지 않게 잘라(clip) 처리한다.

Model 1은 매 시간 하나의 `Streamflow` 예측값을 낸다. Model 2는 같은 시간에 `q50`, `q90`, `q95`, `q99`를 낸다. 여기서 `q`는 quantile(분위수)을 뜻하고, 예컨대 `q95`는 "관측 유량이 이 값보다 낮을 가능성을 95%로 보는 선"이다. Model 2의 `q50`(중앙선)은 Model 1의 단일 예측값과 비교할 대표선으로 쓴다.

> target은 설정 파일의 `target_variables` 항목에 `Streamflow`로 정의하고, 음수 차단은 `clip_targets_to_zero` 항목에서 처리한다.
...
같은 자료를 학습·검증·평가로 나누는 시간 경계는 설정 파일의 날짜 항목으로 정한다. 모델 학습·평가용 공식 구간과, 일부 후속 분석이 기준선을 잡을 때 참조하는 더 넓은 구간을 함께 정리하면 다음과 같다.
...
마지막 행의 "후속 분석 기준 구간"은 모델을 추가로 학습하는 구간이 아니라, 일부 분석이 유역별 임계값을 잡을 때만 참조하는 더 넓은 관측 구간이다. 예를 들어 `q99_analysis`의 유역별 큰 물 기준선(대문자 Q99)은 test 구간이 아니라 학습 기간(2000–2010) 관측 유량의 상위 1% 분위수로 한 번 고정한다. test 기간 관측이 기준선 결정에 끼어들지 않게 하려는 설계로, 정확한 참조 구간은 분석마다 다르므로 각 분석 문서(예: [`10_q99_analysis.md`](10_q99_analysis.md))와 공용 설정 모듈 `expanded_drbc.py`[^src-exp]에서 확인한다.
...
전체 학습 pool을 그대로 학습하면 계산 비용이 매우 크므로, scaling pilot(유역 수를 100/300/600으로 바꿔 가며 적정 규모를 찾는 운영용 사전 실험)을 거쳐 학습·검증 유역 수를 300으로 고정했다. 현재 compute 제약을 반영한 공식 비교 실험(main comparison)이 직접 쓰는 split 구성은 다음과 같다.

- train: 269개 (non-DRBC 학습 유역)
- validation: 31개 (non-DRBC 검증 유역)
- test: 85개 (관측이 확보된 DRBC holdout 유역)

이 split이 여러 층으로 나뉘는 자세한 사정과 각 층의 정확한 숫자는 데이터 처리·분석 가이드 문서[^doc-guide]에 정리되어 있다.

### test 유역을 확장해 재분석한 경위

train·validation split은 학습이 모두 끝난 뒤에 다시 손대지 않는 것이 원칙이다. 그래서 이 연구도 train 269개 / validation 31개 split은 처음 고정한 그대로 유지한다. 다만 학습이 끝난 뒤, 평가 단계에서 DRBC 안에서 실제로 관측 유량이 확보되는 test 유역을 더 넓게 모을 수 있음을 알게 되었다. 그래서 모델을 다시 학습하지 않고 test 유역만 확장해 재평가했다.
...
이 조합 덕분에, forcing만 있어 정답이 없는 것처럼 보였던 DRBC 유역도 별도 관측 유량 원본과 짝지으면 test에 쓸 수 있다. DRBC holdout 후보 154개에 metadata 품질 기준과 2014–2016 target coverage 기준을 적용해 추린 결과가 현재 공식 test 85개다. 이 85개 중 forcing을 `timeseries`에서 가져온 유역이 36개, `timeseries_nonobs`+`Hourly2` 조합으로 복원한 유역이 49개다. 즉 확장은 새 유역을 임의로 더한 것이 아니라, 이미 holdout 후보였던 유역의 관측 유량을 다른 원본에서 되살려 평가 가능 범위를 넓힌 것이다.
...
학습 시에는 모델이 최근 `336시간`(설정 항목 `seq_length`)을 입력으로 보고 마지막 `24시간`(설정 항목 `predict_last_n`)의 유량을 맞히도록 sample을 만든다. 이 336시간 입력 안에 dynamic forcing 결측이 하나라도 있으면 그 sample은 학습에 쓰지 않는다. 반대로 `Streamflow`는 24시간 정답 구간이 전부 비어 있으면 sample을 버리지만, 일부 시간만 비어 있으면 나머지 유효한 시간만 loss(틀린 정도 점수) 계산에 쓴다.
...
`output/basin/all/analysis/` 산출물은 모델 학습 결과가 아니라, 유역과 event를 해석하기 위한 배경 자료다. 서버에서 `.nc` 파일이 모두 준비되면 전 유역 관측 유량 분석 runner `run_camelsh_flood_analysis.sh`[^src-flood]로 생성한다.

[^cfg-m2]: `configs/camelsh_hourly_model2_drbc_holdout_broad.yml`
[^src-exp]: `scripts/_lib/expanded_drbc.py`
[^doc-guide]: `docs/experiment/method/data/data_processing_analysis_guide.md`
[^src-flood]: `scripts/runs/official/run_camelsh_flood_analysis.sh`
