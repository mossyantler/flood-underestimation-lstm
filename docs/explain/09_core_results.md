...
이 문서는 이 연구의 핵심 결과를 비전공 대학생이 한 흐름으로 읽도록 정리한다. 수문학이나 기계학습 배경이 전혀 없어도 따라올 수 있게, 개념을 일상어로 먼저 풀고 나서 숫자를 붙인다. 모든 수치는 실제 분석 결과에서만 가져왔으며, 각 수치가 "어떤 스크립트가 어떤 공식으로 어떤 파일에 써 둔 값인지"를 본문에 함께 적어 둔다. 어디서 나온 숫자인지 추적할 수 있어야 비전공자도 결과를 믿고 읽을 수 있기 때문이다.
...
세 질문의 모든 수치는 한 곳에서 계산된다. 분석 스크립트는 `scripts/model/expanded_drbc/` 폴더에 RQ별로 나뉘어 있고(RQ-1은 `compute_rq1_central_metrics.py`, RQ-2는 `compute_rq2_alpha_peak_deficit.py`·`compute_rq2_beta_window_capture.py`·`compute_rq2_delta_threshold_recall.py`, RQ-3은 `compute_rq3_cost.py`), 산출 표·그림은 모두 `output/model_analysis/primary/metrics/` 아래 `tables/`(수치 표)와 `figures/`(그림)에 저장된다. 입력은 공통으로 seed별 예측·관측을 한 줄씩 모아 둔 `output/model_analysis/primary/metrics/data/required_series/seed{111,222,444}/required_series.csv` 파일이며, 이 파일에는 관측값(열 이름 `obs`), Model 1 예측(열 이름 `model1`), 그리고 Model 2의 네 예측선(열 이름 `q50`, `q90`, `q95`, `q99`)이 시각(열 이름 `datetime`)·유역(열 이름 `basin`)별로 들어 있다.
...
여기서 "큰 물 기준선" Q99는 매번 새로 정하지 않고 한 번 계산해 둔 표를 모든 RQ가 공유한다. 유역마다 다른 이 기준선은 학습 기간(2000~2010년) 관측 유량의 상위 1% 높이로 정해져 `tables/rq2_q99_per_basin_thresholds.csv`(열 이름 `q99_train_value`)에 저장돼 있고, RQ-2의 δ와 RQ-3의 FAR이 이 값을 그대로 읽어 쓴다. 평가 기간(2014~2016년)이 아니라 학습 기간으로 기준선을 잡는 이유는, 모델이 평가 기간을 "미리 엿보고" 기준을 맞추는 일을 막기 위해서다.
...
요약 방식의 "두 번 중앙값" 절차는 모든 스크립트가 똑같이 따른다(코드 주석에 "C0 canonical"이라 적혀 있다). 먼저 한 유역 안에서 seed 세 개의 값을 중앙값으로 묶고, 그렇게 얻은 85개 유역 값을 다시 중앙값으로 묶는다. 이렇게 두 단계로 묶으면 운 좋은(혹은 나쁜) seed 하나, 혹은 유난히 튀는 유역 하나가 전체 결론을 흔드는 일을 줄일 수 있다. 또 모든 스크립트는 관측값이 비어 있는(NaN) 시각을 공통 helper(`scripts/_lib/expanded_drbc.py`의 `filter_valid_rows`)로 먼저 걸러 내고 계산을 시작한다.
...
RQ-1은 `scripts/model/expanded_drbc/compute_rq1_central_metrics.py`가 담당한다. 이 스크립트는 입력 파일에서 관측 열(`obs`), Model 1 예측 열(`model1`), Model 2 중앙선 열(`q50`)만 꺼내, 유역별·seed별로 여섯 지표를 한꺼번에 계산하는 함수(함수 이름 `compute_metrics`)를 돌린다. 여섯 지표의 정의는 코드에 그대로 박혀 있다. NSE는 잔차(예측−관측)의 제곱합을 관측 분산으로 나눠 1에서 뺀 값, RMSE는 잔차 제곱의 평균에 제곱근을 씌운 값, MAE는 잔차 절댓값의 평균, 편향(bias)은 잔차의 평균(즉 평균(예측−관측))으로 계산한다. 고유량 편향 FHV는 별도 함수(함수 이름 `_compute_fhv`)에서 관측과 예측을 각각 큰 값부터 줄세운 뒤 상위 2%(코드 상수 `_FHV_H = 0.02`)만 떼어 그 합끼리 비교해, (예측 상위합 − 관측 상위합) / 관측 상위합 × 100으로 백분율을 낸다. 즉 FHV가 음수면 모델이 가장 큰 유량 구간을 관측보다 낮게 잡았다는 뜻이다.

두 모델을 견주는 차이값(δ가 아니라 여기서는 단순 차이)은 같은 seed끼리 짝지어(`q50` 값 − `model1` 값) 먼저 구한 뒤 seed 중앙값으로 묶는다(코드 함수 `paired_delta_per_seed`). 검산도 코드에 들어 있는데, 새로 계산한 NSE·KGE를 기존에 따로 저장돼 있던 원자료 지표 파일과 맞춰 보아 소수점 6자리(1e-6) 안에서 일치함을 확인했다고 적혀 있다. 결과는 `tables/rq1_central_metrics_pooled_summary.csv`(요약), 유역·seed별 원값은 `tables/rq1_central_metrics_per_basin_seed.csv`, 그림은 `figures/rq1_central_metric_boxplots.png`에 저장된다.
...
### 3-2. 세 가지 잣대: α, β, δ — 참조 카드

세 지표로 잰다. 모두 사건(event), 즉 "큰 물 기준선(Q99)을 관측 유량이 넘었던 한 번의 사례" 단위에서 출발한다(δ만 사건이 아니라 모든 시각을 합쳐 계산한다). 아래는 한눈에 보는 참조 카드이고, 각 지표의 정의·직관·숫자 예시는 바로 이어지는 하위 섹션에서 상세히 푼다.
...
세 지표가 의지하는 "사건 목록"은 미리 만들어 둔 표 `tables/rq2_q99_events_85basin.csv`에 들어 있다(82개 유역에서 일어난 926건의 큰 물 사건). 이 표에는 사건마다 첨두가 일어난 시각(열 이름 `peak_time`)과 그 전후 구간의 시작·끝(열 이름 `window_start`, `window_end`)이 적혀 있어, α는 `peak_time`을, β는 `window_start`~`window_end` 구간을 그대로 읽어 쓴다.
...
**무엇을 재나.** 한 사건에서 관측 첨두를 obs_peak, 그 같은 시각의 예측선 값을 pred_peak라 하면, 부족분은 (obs_peak − pred_peak) / obs_peak로 정의한다(단, 예측이 관측을 넘어선 경우는 0으로 처리). 이 계산은 `scripts/model/expanded_drbc/compute_rq2_alpha_peak_deficit.py`의 함수 `compute_alpha`에 그대로 들어 있다. 코드에서는 사건 목록의 첨두 시각(`peak_time`)에 맞는 예측·관측만 끌어와, 각 예측선 τ별로 `max(0, (obs − pred)) / obs`를 계산하고 그 값을 변수명 `peak_under_deficit`으로 저장한다. 즉 예측이 관측을 넘어서면 음수가 되지 않도록 0에서 잘라 내므로 α는 항상 0과 1 사이에 머문다. 산출 파일은 `tables/rq2_alpha_event_peak_deficit_q99.csv`(전체 사건)와 `tables/rq2_alpha_event_peak_deficit_noaa.csv`(미국 기상청 확인 사건)다.
...
**무엇을 재나.** β는 (첨두 ±6시간 창 안 예측선 최댓값) / (같은 창 안 관측 최댓값)으로 정의한다. 계산은 `scripts/model/expanded_drbc/compute_rq2_beta_window_capture.py`의 함수 `compute_window_capture`가 맡는다. 코드는 사건 목록의 창 시작·끝(`window_start`~`window_end`)으로 구간을 잘라 그 안의 관측 최댓값과 예측선 최댓값을 각각 구한 뒤 그 비율을 변수명 `window_capture`로 저장한다. 관측 최댓값이 0 이하인 사건(예: 댐으로 조절돼 흐름이 거의 없던 구간)은 의미가 없어 빼고, 비율이 2배를 넘으면(코드 상수 `CAPTURE_FLAG_THRESHOLD = 2.0`) 따로 표시해 둔다. α와 달리 β는 위로 잘라 내지 않으므로 1을 넘는 값(과대)이 나올 수 있다. 산출 파일은 `tables/rq2_beta_window_capture_q99.csv`(및 미국 기상청 확인 사건용 `_noaa.csv`)다.
...
**무엇을 재나.** δ는 사건 단위가 아니라 평가 기간 모든 시각을 합쳐(pooled) 계산한다. `scripts/model/expanded_drbc/compute_rq2_delta_threshold_recall.py`의 함수 `compute_recall_for_seed`가, 먼저 관측이 큰 물 기준선(앞서 만들어 둔 `q99_train_value`) 이상인 시각만 골라 그 개수를 분모(변수명 `n_q99_hours`)로 잡고, 그 가운데 예측선이 관측을 넘긴(`q_τ ≥ obs`) 시각 수를 분자(변수명 `n_hits`)로 잡아 그 비율을 변수명 `recall`로 저장한다. 즉 "실제로 큰 물이 났던 시각 중, 예측선이 그 높이를 따라잡은 시각의 비율"이다. 코드에는 분모(`n_q99_hours`)가 세 seed에서 똑같아야 한다는 검산도 들어 있다. 산출 파일은 유역·seed별 `tables/rq2_delta_threshold_recall_per_basin_seed.csv`와 요약 `tables/rq2_delta_threshold_recall_summary.csv`다.
...
세 잣대 모두 같은 방향을 가리킨다. 예측선을 높일수록, α는 줄고(좋아지고), β는 올라가고(좋아지고), δ는 올라간다(좋아진다). 그리고 그 변화가 **단조적(monotone)** 이다 — 중간에 역전이 없다. 이 단조성은 우연이 아니라 세 스크립트 모두 마지막에 "q50 → q90 → q95 → q99 순서로 값이 한 방향으로만 움직이는지"를 검사하는 assert 구문으로 강제하고 있어, 만약 한 군데라도 역전이 생기면 계산이 멈추도록 돼 있다. 실제로 유역별로 따로 봐도 이 순서를 어기는 비율은 0%였다고 α 스크립트에 기록돼 있다.

### 3-5. 미국 기상청(NOAA) 확인 홍수 사건 점검

위의 결과는 "Q99 기준선을 넘은 사건" 전체를 대상으로 했다. 더 엄격한 점검으로, 미국 기상청(NOAA, National Oceanic and Atmospheric Administration)이 실제 홍수로 공식 확인한 사건만 모은 별도 집합(검증 기간에 겹치는 21개 유역, 65개 사건)으로도 같은 분석을 했다. 이 사건 목록은 `tables/rq2_noaa_events_expanded_overlap.csv`에서 읽어 오는데, α·β 스크립트가 이 표를 불러올 때 85개 평가 유역에 드는 사건(`in_expanded_85`)이면서 평가 기간(2014~2016년)에 일어난 사건만 남기도록 코드에서 추려 낸다.
...
두 비용은 모두 `scripts/model/expanded_drbc/compute_rq3_cost.py`의 함수 `compute_cost_for_seed`에서 같이 계산해 각각 `tables/rq3_far_per_basin_seed.csv`와 `tables/rq3_over_prediction_magnitude_per_basin_seed.csv`로 저장한다. 이득(δ 재현율)과 대가(FAR)를 한 그림에 겹쳐 보여 주는 그림은 `figures/rq3_cost_recall_tradeoff.png`다.

### 4-2. FAR — 헛경보 비율 (참조 카드)
...
**무엇을 재나.** FAR은 (예측선이 Q99를 넘었지만 실제 관측은 Q99 미만인 시각 수) / (관측이 Q99 미만인 전체 시각 수)로 정의한다. 코드에서는 먼저 관측이 기준선 미만인 시각만 골라 그 개수를 분모(변수명 `n_below_q99_hours`)로 잡고, 그 가운데 예측선이 기준선을 넘긴 시각 수를 분자로 잡아 비율을 변수명 `far`로 저장한다. 분모가 "Q99 미만인 시각 전체"라는 점을 기억하자. 대부분의 시간은 평소 수위라 FAR의 절댓값이 작게 보일 수 있다.
...
#### 과대예측 크기 — 예측이 실제보다 얼마나 위로 갔는가

**일상 비유.** "오늘 주가가 100만 원 이상 오를 거야"라고 예보했는데 실제로는 50만 원 올랐다면, 과대예측 크기는 50만 원이다. 예측이 실제보다 위로 갔을 때 그 차이의 평균을 구한 것이다.

**무엇을 재나.** 과대예측 크기는 예측선이 관측을 넘긴(`q_τ > obs`) 시각만 골라 그때의 (예측선 − 관측)을 평균한 값이다. 코드는 이 평균을 변수명 `over_pred_magnitude`로 저장하며, 단위는 관측 유량과 같은 mm/hr다. 즉 "얼마나 자주 넘는가"가 아니라 "넘을 때 평균적으로 얼마나 크게 넘는가"를 본다.

**값이 크면**: 예측이 관측보다 훨씬 높이 올라간다. 예측을 보고 대비하면 실제보다 과도한 자원을 투입할 수 있다.
**값이 작으면**: 예측이 관측을 넘더라도 크게 벗어나지 않는다.

### 4-3. 결과 (85개 유역, 유역별 중앙값)
...
예측선을 높일수록 FAR과 과대예측 크기가 함께 올라간다. q50 → q99로 가면 FAR은 약 23배, 과대예측 크기는 약 2.3배 증가했다. RQ-2와 마찬가지로 이 스크립트도 FAR이 q50 → q99에서 한 방향으로만 커지는지를 assert로 검사하므로, 단조 증가가 깨졌다면 계산이 멈췄을 것이다.
