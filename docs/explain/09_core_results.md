# 09. 핵심 결과: 중앙 예측 유지·상위 quantile 첨두·그 대가
...
이 글은 입문용 설명이며 공식 기준 문서가 아니다. 수치·구조의 공식 근거는 `docs/experiment/analysis/model/`의 세 분석 문서(`01_q50_central.md`, `02_upper_quantile_peak_under.md`, `03_cost.md`)가 보관한다. 연구 질문 전체 지도(RQ-0~5, RQ-4가 4a·4b로 갈려 질문 7개)는 [`08_rq_analysis_map.md`](08_rq_analysis_map.md)에 있다.
...
이득만 보고하고 비용을 숨기면 균형 잡힌 평가가 아니다. 홍수 예보 맥락에서 위경보는 실제 비용이 있다. 주민이 불필요하게 대피하거나, 경보를 반복해서 잘못 울리면 나중에 진짜 경보를 무시하는 "경보 피로"가 생긴다. 비유하자면 암 검진에서 위양성을 줄이려면 판정 기준을 느슨하게 잡아야 하는데, 그러면 암 환자를 더 많이 잡는 대신 불필요한 정밀 검사도 늘어나는 것과 같다.
