...
/// 이렇게 하나의 주제를 말하는 제목인 heading에 문장으로 끝나는 것을 싫어함. 알아두었으면 좋겠음.
...
두 모델 사이에서 **핵심적으로 바뀐 것은 출력 방식 하나**다. 학습 데이터, 유역 조건, LSTM 구조, 학습 기간은 모두 동일하다. 이 차이는 단순히 글로만 정한 것이 아니라 설정 파일에 그대로 적혀 있다. Model 1은 `configs/camelsh_hourly_model1_drbc_holdout_broad.yml`에서 출력층(설정 항목 `head`)을 일반 회귀 방식인 `regression`으로, 학습 기준(설정 항목 `loss`)을 `nse`로 두고, Model 2는 `configs/camelsh_hourly_model2_drbc_holdout_broad.yml`에서 출력층을 `quantile`로, 학습 기준을 `pinball`로 두며 내보낼 분위 목록(설정 항목 `quantiles`)에 0.5, 0.9, 0.95, 0.99를 적는다. 이렇게 출력 한 군데만 다르게 묶어 두기 때문에, 결과 차이를 "출력 설계의 차이"로 깨끗하게 해석할 수 있다.
...
RQ를 보기 전에, 모든 분석이 똑같이 깔고 가는 실험 조건을 확인한다. 이 조건이 흔들리면 "Model 2가 더 낫다"는 비교 자체가 공정하지 않기 때문이다. 그래서 이 조건들은 분석할 때마다 다시 정하지 않고, `scripts/_lib/expanded_drbc.py` 한 파일에 상수로 못 박아 두고 모든 스크립트가 거기서 같은 값을 불러 쓴다. 이렇게 "용어와 기준을 한 군데에 잠가 두는" 방식을 코드 주석은 vocabulary lock(용어 자물쇠)이라고 부른다.
...
이 네 가지는 코드 안에 다음처럼 들어 있다. 세 개의 seed는 변수명 `SEEDS`에 `(111, 222, 444)`로, 학습 기간은 변수명 `TRAIN_PERIOD`에 `2000-01-01 ~ 2010-12-31`로, 평가 기간은 변수명 `TEST_PERIOD`에 `2014-01-01 ~ 2016-12-31`로, 큰 유량을 가르는 상위 1% 기준은 변수명 `HIGH_FLOW_PERCENTILE`에 `0.99`로 적혀 있다.

**Q99란 무엇인가.** 학습 기간(2000-2010) 동안 그 유역에서 관측된 시간별 유량 전체를 크기 순서로 세웠을 때 상위 1%에 해당하는 값이다. 비유하자면 반에서 100명이 시험을 봤을 때 1등부터 99등까지는 Q99 아래이고, 1등(가장 높은 유량)만 Q99를 넘는다. 유역마다 이 기준이 다르기 때문에 "작은 강에서 큰 홍수", "큰 강에서 큰 홍수"를 같은 기준으로 비교할 수 있다. 실제로 이 값은 `build_q99_events.py` 스크립트가 각 유역의 학습 기간 유량(NetCDF 파일의 `Streamflow` 변수)을 모아 99% 분위(`np.quantile(..., 0.99)`)로 계산하고, 그 결과를 `rq2_q99_per_basin_thresholds.csv`라는 표에 유역별로 한 줄씩 저장한다.
...
| ----- | --------------------------------------- | ------------------- | ------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| RQ-0  | 여러 예측선을 동시에 어떻게 읽을 것인가 | 방법(틀)            | `quantile_output_interpretation.md` | (용어 자물쇠:`expanded_drbc.py`)                                                                                                                                           |
...
분석 문서는 모두 `docs/experiment/analysis/model/` 안에 있다. `quantile_output_interpretation.md`만 `docs/experiment/method/model/`에 있다. 계산 스크립트는 RQ-5용 한 개(`scripts/model/hydrograph/`)를 빼면 모두 `scripts/model/expanded_drbc/` 폴더에 있고, 표에서는 폴더 경로를 줄여 파일명만 적었다. 스크립트가 만들어 내는 표·그림은 모두 `output/model_analysis/expanded_drbc_test/` 아래의 `tables/`(수치 표)와 `figures/`(그림) 폴더에 떨어진다.
...
이 약속이 말로만 끝나지 않도록, 분위의 순서(`model1 → q50 → q90 → q95 → q99`)와 용어들은 `scripts/_lib/expanded_drbc.py`에 상수로 잠가 둔다. 분위 순서는 변수명 `TAU_ORDER`에, 예측선마다 표에서 어느 열을 읽을지는 변수명 `PREDICTION_COLUMNS`에 적혀 있다. 뒤따르는 모든 RQ 스크립트는 이 순서와 열 이름을 직접 정의하지 않고 이 파일에서 가져다 써야 한다.
...
분석 문서 `01_q50_central.md`가 이 질문을 담당하고, 실제 계산은 `compute_rq1_central_metrics.py` 스크립트가 한다. 이 스크립트는 85개 유역 각각에서 seed 세 개를 돌린 예측-관측 시계열(시점별로 예측과 실제 유량이 나란히 적힌 표, 변수명 `required_series`)을 읽어, Model 1과 Model 2 `q50`의 성능 지표를 같은 자리에서 계산한 뒤 짝지어 비교(paired comparison)한다. 짝지어 비교란, 같은 유역·같은 seed 안에서 두 모델 점수의 차이를 먼저 구하고(코드의 변수명 `delta`), 그 차이를 유역별로 세 seed에 대해 중앙값으로 모으는 방식이다.

스크립트는 NSE·KGE·bias·MAE·RMSE·FHV 여섯 지표를 계산하지만, 이 글에서는 비전공 독자가 직관적으로 읽을 수 있는 세 가지(NSE·RMSE·MAE)만 추려 본다.

| 심볼 | 변수명   | 범위                   | 최적화 방향      |
| ---- | -------- | ---------------------- | ---------------- |
| NSE  | `nse`  | 1이 최선 (음수도 가능) | 클수록 좋음 ↑   |
| RMSE | `rmse` | 0 이상 (0이 최선)      | 작을수록 좋음 ↓ |
| MAE  | `mae`  | 0 이상 (0이 최선)      | 작을수록 좋음 ↓ |

##### NSE — 적합도 지표 (Nash-Sutcliffe Efficiency)

모든 시점에서 "그냥 전체 평균을 예측했을 때의 오차"를 기준으로 놓고, 모델이 그것보다 얼마나 나은지를 점수로 나타낸 것이다. NSE가 1이면 완벽, 0이면 그냥 평균을 찍은 것과 같고, 음수면 평균을 찍는 것보다도 나쁘다는 뜻이다. NSE가 0.7이면 "그냥 평균 예측보다 오차를 70% 줄인 것", 0.9면 "90% 줄인 것"으로 읽을 수 있다. 수문 모델에서 가장 표준적으로 쓰는 점수다.

##### RMSE — 평균 제곱근 오차 (Root Mean Square Error)

예측이 실제 관측에서 평균적으로 얼마나 벗어나는지를 유량과 같은 단위로 나타낸 값이다. 코드에서는 `sqrt(mean((pred − obs)^2))`로 계산한다. 큰 오차에 더 무겁게 벌점을 주기 때문에, 가끔씩 크게 빗나가는 예측에 민감하다. 작을수록 좋다.

##### MAE — 평균 절대 오차 (Mean Absolute Error)

예측 오차의 절댓값을 단순 평균한 값으로, 코드에서는 `mean(|pred − obs|)`로 계산한다. RMSE와 달리 큰 오차에 특별히 더 벌점을 주지 않아, "평소에 얼마나 빗나가는가"를 더 고르게 보여 준다. 작을수록 좋다.
...
1. **Q99 초과 사건**: 관측 유량이 그 유역의 Q99(학습 기간 상위 1% 기준)를 넘어선 시간 구간. `build_q99_events.py`가 평가 기간(2014-2016) 안에서 관측 유량이 Q99 이상인 시점을 찾고, 12시간(변수명 `EVENT_MERGE_GAP_HOURS`) 안에 붙어 있는 시점들을 하나의 사건으로 묶은 뒤, 그 사건의 꼭대기 시점(`peak_time`)을 잡아 `rq2_q99_events_85basin.csv`에 기록한다.
2. **NOAA 확인 홍수 사건**: NOAA(National Oceanic and Atmospheric Administration, 미국 해양대기청)가 공식 홍수로 기록한 실제 사건. `build_noaa_mapping.py`가 NOAA 공식 홍수 목록을 85개 유역에 맞춰 연결한다.
...
분석 문서 `02_upper_quantile_peak_under.md`가 담당한다. 세 가지 지표(α, β, δ)로 답하며, 이 세 가지를 묶어 "지표 세트(metric triplet)"라 부른다. 각 지표는 서로 다른 스크립트가 계산한다. α는 `compute_rq2_alpha_peak_deficit.py`, β는 `compute_rq2_beta_window_capture.py`, δ는 `compute_rq2_delta_threshold_recall.py`가 맡는다.

| 심볼 | 변수명                 | 범위                        | 최적화 방향      |
| ---- | ---------------------- | --------------------------- | ---------------- |
| α   | `peak_under_deficit` | 0~1 (0이 최선)              | 작을수록 좋음 ↓ |
| β   | `window_capture`     | 0 이상 (1=딱 맞음, >1=과대) | 1 근처/↑        |
| δ   | `recall`             | 0~1                         | 클수록 좋음 ↑   |

/// 심볼 이전에 그 심볼의 명칭이 무엇인지 작성해라. 지표 세트니까 `지표` 이라고 하면 될 듯.

##### α — 첨두 부족분 (event peak under-deficit)

각 사건에서 "예측 꼭대기가 실제 꼭대기보다 얼마나 낮은가"를 비율로 잰다. `compute_rq2_alpha_peak_deficit.py`는 각 사건의 꼭대기 시점에서 실제 유량과 예측 분위 값을 꺼내, 다음과 같이 계산한다(코드의 구현식 그대로).

```
α = max(0, (실제 꼭대기 − 예측 꼭대기)) / 실제 꼭대기
```

/// 수식일 경우 math block 사용. 아래도 마찬가지.

`max(0, ...)`을 씌우기 때문에, 예측이 실제 꼭대기보다 높게 잡힌 경우는 0으로 처리한다(이 지표는 "낮게 잡은 정도"만 본다). α가 0이면 예측이 실제 꼭대기를 정확히 맞혔거나 더 높게 잡았다는 뜻이고, α가 0.5면 예측이 실제 꼭대기의 절반 높이밖에 안 됐다는 뜻이다. α가 클수록 과소추정이 심하고, 작을수록 꼭대기를 잘 따라잡은 것이다.

비유하자면, 실제 홍수 수위가 10m까지 올랐는데 모델이 7m라고 예측했다면 α = 0.3이다. 모델이 9.8m라고 예측했다면 α = 0.02다.

##### β — 꼭대기 시점 전후 포착 비율 (±6h window capture)

실제 꼭대기가 발생한 시점 앞뒤 ±6시간( `EVENT_WINDOW_HOURS`)이라는 시간 창 안에서, "그 창 안 예측 최댓값이 그 창 안 실제 최댓값의 몇 배인가"를 비율로 잰다. `compute_rq2_beta_window_capture.py`의 구현식은 다음과 같다.

```
β = (창 안 예측 최댓값) / (창 안 실제 최댓값)
```

즉 β는 0/1로 잡았냐/못 잡았냐를 나누는 깃발이 아니라 연속적인 비율이다. β가 1이면 그 시간대 어딘가에서 예측이 실제 꼭대기 높이에 딱 도달했다는 뜻이고, 1보다 작으면 그만큼 못 미친 것, 1보다 크면 오히려 더 높게 잡은 것이다. α가 꼭대기 "바로 그 순간"만 본다면, β는 꼭대기 "근처 ±6시간"으로 시간 여유를 줘서 본다는 점이 다르다. 실제 꼭대기가 0 이하인 사건(규제 하천 등)은 비율을 잴 수 없어 계산에서 제외한다.

##### δ — Q99 초과 시점 회수율 (pooled threshold recall)

실제로 유량이 Q99를 넘은 모든 시점만 모은 다음, 그중에서 예측 분위 값이 실제 관측값 이상으로 따라 올라간 시점의 비율이다. `compute_rq2_delta_threshold_recall.py`의 정의는 정확히는 다음과 같다.

```
δ = P(예측 분위 ≥ 실제값 | 실제값 ≥ Q99)
```
...
분석 문서 `03_cost.md`가 담당하고, 계산은 `compute_rq3_cost.py`가 한다. 이 스크립트는 평가 기간 전체 시점을 유역·seed·분위별로 훑으면서 두 가지 비용 지표를 계산한다.

| 심볼          | 변수명                  | 범위               | 최적화 방향      |
| ------------- | ----------------------- | ------------------ | ---------------- |
| FAR           | `far`                 | 0~1 (0이 최선)     | 작을수록 좋음 ↓ |
| 과대예측 크기 | `over_pred_magnitude` | 0 이상 (유량 단위) | 작을수록 좋음 ↓ |

##### FAR — 헛경보 비율 (False Alarm Ratio)

실제로는 유량이 Q99 아래였는데 예측이 Q99를 넘었다고 알린 비율이다. `compute_rq3_cost.py`의 정의는 다음과 같다.

```
FAR = P(예측 분위 > Q99 | 실제값 < Q99)
```
...
예측이 실제 관측보다 클 때, 그 초과분의 평균이다. 구현식은 `mean(예측 분위 − 실제값 | 예측 분위 > 실제값)`으로, 예측이 실제보다 높게 나온 시점만 모아 그 차이를 평균한다. 예측이 얼마나 크게 과장됐는지를 유량 단위로 잰다. 작을수록 좋다.
...
나누는 기준은 **Model 1의 NSE 성능을 기준으로 상·중·하 세 단계(tier)로 나누기**이다. `compute_rq4a_nse_tier_stratify.py`는 RQ-1에서 계산한 Model 1의 유역별 NSE(세 seed 중앙값)를 가져와, `pd.qcut`이라는 도구로 위에서부터 정확히 1/3씩 잘라 top·mid·bottom 세 단계를 만든다.
...
유형 분류는 사람이 손으로 붙이는 것이 아니라, NOAA 사건 설명 문구에 어떤 단어가 들어 있는지를 정규식(regex, 글자 패턴 검색)으로 찾아 자동으로 붙인다. 이 단어 사전과 패턴은 `scripts/_lib/expanded_drbc.py`의 변수명 `NOAA_LABELS`(사용 가능한 유형 목록)와 `NOAA_REGEX`(유형별 검색 패턴)에 잠겨 있고, 한 사건 문구에 여러 유형 단어가 같은 횟수로 나오면 변수명 `NOAA_TIE_BREAK` 순서(가장 구체적인 유형이 우선)로 대표 유형을 정한다. 실제 분류 작업은 `build_noaa_mapping.py`가 하고, 그 결과를 받아 유형별 성능을 비교하는 일은 `compute_rq4b_event_type_stratify.py`가 한다.
...
| pinball loss   | `pinball`            | 0 이상 (0이 최선)            | 작을수록 좋음 ↓             |
