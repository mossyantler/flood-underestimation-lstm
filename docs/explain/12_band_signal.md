...
전제는 다른 장과 같다. DRBC(Delaware River Basin Commission, 델라웨어 강 유역 관리 위원회) 지역에서 학습에 쓰지 않고 따로 떼어 둔(holdout) 관측 가능 유역 85개, seed(난수 씨앗) `111 / 222 / 444`, test 기간 2014–2016, temporal split `train 2000–2010 / validation 2011–2013 / test 2014–2016`이다. Q99/NOAA 사건의 관측 위치 구간은 seed별로 먼저 계산한 뒤 사건 단위 중앙값을 primary로 쓰고, 전체강우 대조 범위는 seed 평균 시계열에서 위치를 계산한다.

이 분석의 산출물과 그림은 모두 `output/model_analysis/band_signal/` 아래에 모여 있고, 계산 스크립트는 `scripts/model/expanded_drbc/` 폴더에 있다. 본문에서 각 수치의 출처 스크립트를 그때그때 짚는다.
...
먼저 "quantile(분위수)"의 뜻부터 짚는다.

> **quantile(분위수)란?** 어떤 값 x에 대해 "관측이 x보다 작을 확률이 몇 퍼센트인지"를 나타내는 눈금. 예를 들어 q90은 "관측이 이 값보다 낮을 확률이 90%"라는 뜻으로 모델이 그어 두는 선이다. 즉 q90 아래에 관측이 들어올 것을 90% 확률로 기대한다.
...
이 네 값은 모델이 매 시각 함께 내보내는 출력이며, 분석에서는 seed별 예측 시계열 파일(예측 결과를 시간순으로 적어 둔 표)에 다음 이름의 열로 저장된다. 같은 파일에 실제 관측 유량도 `obs` 열로 들어 있어, 한 시각에서 "관측이 네 예측선 중 어디에 들어왔는지"를 곧바로 따진다.
...
홍수가 지나간 뒤 실제 관측 기록을 확인한다. 관측된 첨두유량(홍수 중 가장 큰 순간의 물 흐름)이 사다리의 어느 칸에 들어왔는지 보면, 모델이 얼마나 잘 "감쌌는지"가 드러난다. 관측 첨두 위치는 네 가지 상황으로 갈린다.
...
이 칸 나누기는 글로만 정한 약속이 아니라 코드에 그대로 구현돼 있다. 관측 유량을 네 예측선과 차례로 비교해 다섯 이름 중 하나를 붙인다. (아래 수식에서 $Q$는 유량을 뜻하고, 아래첨자 $\text{obs}$는 실제 관측, $\text{sim},\tau$는 분위 $\tau$의 모델 예측을 가리킨다. 본문 산문에서는 "관측", "예측"으로 풀어 쓴다.)
...
번호가 **높을수록** 실제 첨두가 사다리 위쪽에 들어왔다는 뜻이며, 모델이 첨두를 **과소추정**했다는 신호다. 번호가 **낮을수록**(단, 0은 예외) 모델 밴드가 관측 첨두를 넉넉히 감쌌다는 뜻이다. 0번(`below_q50`)은 방향이 반대인 실패다. 위쪽으로 못 잡은 게 아니라 중앙선조차 실제보다 높게 잡은 과대 예측이기 때문이다.
...
모델이 낸 가장 낮은 대표선(q50)보다도 실제 유량이 더 낮았던 경우. "모델이 첨두를 놓쳤다"가 아니라 반대로 "모델이 평소 유량을 너무 높게 잡았다"는 신호다. 다섯 칸 중 유일한 과대 쪽 실패다.
...
가장 위로 열어 둔 q99마저 관측이 넘어버린 경우. 이 연구가 가장 주목하는 첨두 과소추정 사건이며, 좋은 모델일수록 이 칸의 비율이 작아야 한다. 단, 비율을 한 가지 방식으로만 읽지 않는다. Q99/NOAA 사건은 seed별 위치 구간을 보존한 뒤 사건 단위 중앙값으로 단일 class를 정하고, 유역별 중앙 비율과 event-pooled 비율을 함께 확인한다. 이 `above_q99` 비율이 낮을수록 좋지만, q99가 덮은 사건에서는 얼마나 여유 있게 덮었는지, 못 덮은 사건에서는 얼마나 크게 부족했는지도 같이 본다.
...
q99가 관측보다 훨씬 높게 잡혔다면, 그것은 "큰 홍수다"라는 신호를 과도하게 내는 것과 같다. 홍수 경보를 자주 지나치게 크게 내면 대피·대응 자원이 낭비된다. 신호 해석 기준을 만든 스크립트 `build_quantile_ladder_signal_explainer_html.py`[^src-explainer]도 q99를 두 층으로 본다.
...
관측값으로 계산한 신호를 미래에도 쓸 수 있다는 해석은 틀렸다. 관측 첨두 시점이나 관측 첨두 크기로 계산한 신호는 미래 사건에 쓸 수 없다. 미래 사건에는 아직 관측값이 없기 때문이다. 이것을 **관측값 누수**(내부 코드명 leakage)라고 부른다.
...
신호 탐색에 앞서 어떤 신호가 진짜 단서이고 어떤 것이 착각인지 구분 기준을 먼저 정리한다.
...
> **Spearman 순위상관(r)이란?** 두 값이 같은 순서로 움직이는 정도를 -1에서 +1 사이 숫자로 나타낸 것. r = +1이면 한 값이 커질 때 다른 값도 항상 커진다. r = 0이면 두 값이 서로 무관하다. r = -1이면 한 값이 커질 때 다른 값은 항상 작아진다. "Spearman"이라는 이름은 크기 자체가 아닌 순위(몇 번째로 큰지)를 기준으로 계산한다는 뜻이다.

여기서 각 신호의 r은 "신호 값"과 "관측 위치 구간 번호(`obs_class_ordinal`, 0~4)"가 같은 방향으로 움직이는 정도다. 즉 r이 클수록 그 신호가 클 때 관측이 사다리 위쪽 칸에 들어가는 경향이 강하다.
...
| 투수성 (`permeability`) | +0.32 | +0.04 | +0.26 |
| 기저유출 지수 (`baseflow_index`) | +0.20 | **+0.44** | +0.20 |
| 대류성 강수비 (`crainf_frac`) | +0.05 | **+0.39** | +0.13 |
| CAPE (대류 잠재력) | +0.03 | +0.22 | +0.12 |

### 유역 면적(`area`): 가장 꾸준한 단서

유역 면적은 극단 사건과 NOAA 확인 홍수에서 r = +0.50, 전체 강우 사건에서도 r = +0.27로 양의 값을 유지한다. **큰 유역일수록 관측 첨두가 사다리 위쪽 칸에 들어가는 경향**이 있다는 뜻이다.
...
대류성 비란 무더운 여름날 오후에 좁은 지역으로 갑자기 퍼붓는 소나기다. 짧은 시간에 좁은 면적으로 엄청난 양을 쏟아내며, 지형·시간 분포가 매우 불규칙하다. LSTM 모델이 학습한 "일반적인 강우-유량 패턴"과 다르기 때문에, 대류성 폭우가 내렸을 때 모델이 첨두를 낮게 잡을 가능성이 크다.

CAPE는 대기 중에 저장된 에너지가 얼마나 폭발적으로 비를 만들 수 있는지를 나타내는 기상 지표다. CAPE가 높은 날은 대류성 폭우 가능성이 높고, 그만큼 모델이 못 잡는 상황도 잦아진다.
...
예측 밴드 폭에서 파생된 신호들이 후보로 등장한다. 이 신호들은 관측값 없이 예측선만으로 구할 수 있어 미래 사건에도 계산 가능한 것처럼 보인다.
...
극단 사건(Q99 초과)만 모아 보면 `rel_width`가 r = -0.14로 **음수**가 나왔다. "밴드가 넓을수록 관측이 위쪽 칸에 덜 들어간다"는 뜻이 되는데, 이게 직관과 정반대다.
...
사다리가 넓게 벌어질 때는 q99가 이미 충분히 높아 관측 첨두가 사다리 안에 잘 들어온다(위치 번호 낮아짐). 반대로 사다리가 좁게 그려질 때는 q99도 낮아 관측이 q99를 초과하기 쉽다(위치 번호 높아짐). 그러면 "밴드 넓음 → 위치 낮음"이라는 음의 관계처럼 보인다. 그런데 이것은 **정의 자체에서 생기는 구조적 결합**이지 진짜 예측력이 아니다.
...
결정적 증거: 전체 강우 사건(n ≈ 16,639)으로 범위를 넓히자 `rel_width`의 r이 Q99 −0.14, NOAA −0.16에서 **+0.01로 무너졌다.** 즉 극단 사건에서만 보이던 그 관계는 **극단만 골라 본 데서 생긴 착시(선택 편향)**였다.
...
홍수가 발생할 때 유량이 갑자기 치솟는 구간이 있다. 비가 시작되고 물이 강으로 몰려들며 유량이 빠르게 오르는 이 구간이 **상승부**(rising limb)다. 상승부 기울기는 이 구간에서 단위 시간당 유량이 얼마나 빠르게 오르는지를 나타낸다.
...
`band_slope_signal` 분석은 813개의 깨끗한 홍수 상승 구간에서 이를 검증했다. 계산은 스크립트 `compute_quantile_rise_slope_signal.py`가 맡는다. 이 스크립트는 상승 구간 목록(상승부 사건을 정리한 표, 변수명상 manifest)을 읽어, 각 사건마다 시작점부터 첨두까지 예측선이 시간당 얼마나 올랐는지 계산한 뒤 그 값과 관측 위치 구간 번호의 Spearman r을 잰다.
...
예측선 기울기가 음수로 나오는 데도 구조적 이유가 있다. 그중 가장 강한 것이 q99와 q50 기울기 차이로 정의한 사다리 벌어짐 기울기(`fanning_slope`)이지만 그래도 r = -0.182에 그친다.

관측 위치 구간은 "(관측 첨두) − (예측선)" 차이가 클수록 높은 번호다. 예측 quantile 기울기가 가파를수록 예측선 자체가 높아진다. 예측선이 높아지면 관측이 그것을 넘기 어렵다. 결과적으로 예측 기울기가 크면 → 위치 구간 번호가 낮아지는 음의 관계가 생긴다. 진짜 신호라기보다 다시 정의상의 구조적 결합이다.
...
신호를 극단 사건에서만 보면 착각하기 쉽다는 것을 앞서 지적했다. 그래서 분석은 서로 다른 세 범위에서 같은 신호를 비교했다. 세 범위 비교는 스크립트 `signal_sweep_branchB2_allrain.py`와 `plot_signal_sweep_3scope.py`가 표(`tables/branchB2_spearman.csv`)와 그림(`figures/signal_sweep_3scope.png`)으로 만든다.
...
- **상대 벌어짐(`rel_width`), q99/q50 비율**: Q99 −0.14, NOAA −0.16에서 전체 강우 +0.01로 붕괴 → 정의상 결합과 선택 편향 착시.
- **강우 총량**: Q99 −0.24, NOAA +0.08, 전체 강우 −0.03으로 방향이 안정적이지 않음 → 모델 반응을 통한 간접 결합.
...
상승부 기울기를 재려면 상승 시작 시점을 먼저 정해야 한다. 시작점을 너무 늦게 잡으면 상승 시간이 1~2시간으로 과소평가되고, 너무 일찍 잡으면 이전 사건의 물 빠짐 구간까지 끌어들여 유역 반응 특성이 왜곡된다. 스크립트 `compare_rising_limb_methods.py`는 한 DRBC 유역(`0142400103`)에서 세 검출법을 비교했다.
...
분석은 방법 3(Savitzky-Golay)을 공식 채택했다. 해당 유역 181개 사건 중 97.8%(177개)에서 직접 검출에 성공했고, 검출 실패 4건만 방법 1로 대체(fallback)했다. 채택된 검출법은 공용 helper `camelsh_flood_analysis_utils.py`의 `find_rising_limb_onset()`으로 통합하고 방법 1을 fallback으로 유지하도록 정리됐다.

---

## 13장. 최종 해석: 단정 대신 위험 단계

새 홍수가 닥쳤을 때는 관측값이 없으므로 "이 사건은 칸 3(`q95_to_q99`)에 있다"고 단정할 수 없다. 대신 신호 지표들을 종합해 "위쪽 칸에 들어갈 가능성이 어느 정도인지"를 **위험 단계**(내부 코드명 risk tier)로 표현한다. 위험 단계의 기준선(예: 검증 자료 안 상위 10%·5%·1% 경계)은 DRBC test를 보고 정하지 않고 검증 자료에서 먼저 정해 test에 그대로 적용한다. 이 기준 정의는 신호 해석 기준 문서를 만든 스크립트 `build_quantile_ladder_signal_explainer_html.py`[^src-explainer]에 담겨 있다.
...
| 분류 | 신호 | 결론 |
| --- | --- | --- |
| 안정적 독립 신호 | 유역 면적 (Q99/NOAA r = +0.50, 전체강우 r = +0.27) | 큰 유역일수록 위쪽 칸 경향 |
...
[^src-explainer]: `scripts/model/expanded_drbc/build_quantile_ladder_signal_explainer_html.py`
