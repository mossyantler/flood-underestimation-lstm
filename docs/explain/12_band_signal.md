...
가장 위로 열어 둔 q99마저 관측이 넘어버린 경우다. 이 연구가 가장 주목하는 첨두 과소추정 사건이며, 좋은 모델일수록 이 칸의 비율이 작아야 한다. 칸별 사건 비율은 합이 1이 되도록 모으며(변수명 `pooled_fraction`), 이 `above_q99` 비율이 전체 유역 중앙값으로 0.50 미만이어야 "밴드가 그래도 쓸 만하다"고 본다.
...
q99가 관측보다 훨씬 높게 잡혔다면, 그것은 "큰 홍수다"라는 신호를 과도하게 내는 것과 같다. 홍수 경보를 자주 지나치게 크게 내면 대피·대응 자원이 낭비된다. 신호 해석 기준을 만든 스크립트 `build_quantile_ladder_signal_explainer_html.py`도 q99를 두 층으로 본다. 관측을 덮었는지 보는 포함 여부 층(설명 문서 용어 inclusion guardrail)과, 덮었더라도 얼마나 과도했는지를 재는 거리·비용 층이다. 과대 비용은 다음처럼 정의한다.
