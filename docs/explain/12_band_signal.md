...
가장 위로 열어 둔 q99마저 관측이 넘어버린 경우. 이 연구가 가장 주목하는 첨두 과소추정 사건이며, 좋은 모델일수록 이 칸의 비율이 작아야 한다. 칸별 사건 비율은 합이 1이 되도록 모으며(변수명 `pooled_fraction`), 이 `above_q99` 비율이 전체 유역에서 충분히 낮게 유지돼야 "밴드가 그래도 쓸 만하다"고 본다.
...
유역 면적이 r = +0.43으로 가장 강하고, 극단 사건·NOAA 확인 홍수·전체 강우 사건 모두에서 양의 값을 유지한다. **큰 유역일수록 관측 첨두가 사다리 위쪽 칸에 들어가는 경향**이 있다는 뜻이다.
