...
이 네 값은 모델이 매 시각 함께 내보내는 출력이고, 분석에서는 seed별 예측 시계열 파일(예측 결과를 시간순으로 적어 둔 표, `output/model_analysis/primary/metrics/data/required_series/seed{111,222,444}/required_series.csv`)에 `q50 · q90 · q95 · q99`라는 이름의 열로 저장된다. 같은 파일에 실제 관측 유량도 `obs`라는 이름의 열로 들어 있어, 한 시각에서 "관측이 네 예측선 중 어디에 들어왔는지"를 곧바로 따질 수 있다.
...
이 칸 나누기는 글로만 정한 약속이 아니라 스크립트 `scripts/model/expanded_drbc/compute_ub_location_class.py`의 `classify_obs()` 함수에 그대로 구현돼 있다. 이 함수는 관측 유량(`obs`)을 네 예측선과 차례로 비교해 다섯 이름(`below_q50`, `q50_to_q90`, `q90_to_q95`, `q95_to_q99`, `above_q99`) 중 하나를 붙인다. 붙인 결과는 변수명 `obs_class`로 저장되고, 0~4 번호로 바꾼 값은 변수명 `obs_class_ordinal`로 같이 둔다. 산출 표는 `output/model_analysis/band_signal/band_shape/tables/ub_location_class_q99.csv`(극단 사건)와 같은 폴더의 `_noaa.csv`(NOAA 확인 홍수)에 들어 있다. 아래는 그 다섯 칸을 빠르게 참조하는 카드다.
...
가장 위로 열어 둔 q99마저 관측이 넘어버린 경우다. 이 연구가 가장 주목하는 첨두 과소추정 사건이며, 좋은 모델일수록 이 칸의 비율이 작아야 한다. `compute_ub_location_class.py`는 칸별 사건 비율을 합이 1이 되도록 모은 뒤(변수명 `pooled_fraction`), 이 `above_q99` 비율이 전체 유역 중앙값으로 0.50 미만이어야 "밴드가 그래도 쓸 만하다"고 본다.
...
q99가 관측보다 훨씬 높게 잡혔다면, 그것은 "큰 홍수다"라는 신호를 과도하게 내는 것과 같다. 홍수 경보를 자주 지나치게 크게 내면 대피·대응 자원이 낭비된다. 신호 해석 기준을 만든 스크립트 `build_quantile_ladder_signal_explainer_html.py`도 q99를 두 층으로 본다. 관측을 덮었는지 보는 포함 여부 층(설명 문서 용어 inclusion guardrail)과, 덮었더라도 얼마나 과도했는지를 `max(q99 − 관측, 0) / 관측`으로 재는 거리·비용 층이다.
...
이 세 분류는 신호 탐색 스크립트 `signal_sweep_branchA_csv.py`의 머리말 주석에도 같은 뜻으로 적혀 있다(독립 신호를 코드 주석에서 `I = 독립(누수 없음)`으로, 유역 정적 특성을 변수 묶음 `ATTR_COLS`로 표기한다). `ATTR_COLS`에는 유역 면적·경사·건조도·적설 비율·토양 깊이·투수성·기저유출 지수·산림 비율이 변수명 `area · slope · aridity · snow_fraction · soil_depth · permeability · baseflow_index · forest_fraction`으로 들어 있다.

---

## 8장. 독립 신호: 진짜로 쓸 수 있는 단서

신호 탐색 분석(`band_signal_sweep`)은 Q99 초과 사건 약 926개, NOAA(미국 기상청)가 공식 확인한 홍수 약 65개, 그리고 2014-2016 전체 강우 사건 약 16,639개에 대해 후보 신호와 관측 위치 구간 사이의 연관 강도를 측정했다. 극단 사건·NOAA 사건의 후보 신호는 `signal_sweep_branchA_csv.py`와 `signal_sweep_branchB_forcing.py`가, 전체 강우 사건은 `signal_sweep_branchB2_allrain.py`가 계산한다.
...
예측 밴드 폭에서 파생된 신호들이 후보로 등장한다. 이 두 신호는 스크립트 `compute_ub_band_shape.py`의 `compute_band_shape()` 함수가 계산하며, 관측값 없이 예측선만으로 구할 수 있다.
...
같은 스크립트는 사다리 위쪽이 얼마나 무거운지를 보는 꼬리 지표(변수명 `g3_ratio`, 계산식 (q99 − q95) / (q99 − q50))도 함께 낸다. 이 값들은 "사다리가 위로 많이 벌어질수록 관측 첨두가 높은 칸에 들어올 것"이라는 직관과 연결된다. 그런데 실제 분석 결과는 반대다.
...
기울기를 재려면 먼저 "상승부가 언제 시작했는가"를 정해야 한다. 이 시작점(내부 용어 onset) 검출법은 별도 분석(`method_compare`)에서 세 가지를 비교했다. 자세한 내용은 12장 끝의 별도 절에서 다룬다.

### 분석 결과

`band_slope_signal` 분석은 813개의 깨끗한 홍수 상승 구간에서 이를 검증했다. 계산은 스크립트 `compute_quantile_rise_slope_signal.py`가 맡는다. 이 스크립트는 상승 구간 목록(상승부 사건을 정리한 표, 변수명상 manifest, 파일 `output/model_analysis/band_signal/method_compare/data/rise_h_windows/rise_h_window_manifest.csv`)을 읽어, 각 사건마다 시작점부터 첨두까지 예측선이 시간당 얼마나 올랐는지를 계산한 뒤, 그 값과 관측 위치 구간 번호의 Spearman r을 잰다.
