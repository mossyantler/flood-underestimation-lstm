...
현재 서버 실행 진입점은 `scripts/official/run_camelsh_flood_analysis.sh`다. 이 runner는 먼저 CAMELSH hourly record 기반 Gumbel annual-maxima proxy로 `return_period_reference_table.csv`를 만들고, 그다음 POT 방식의 `event_response_table.csv`를 만든 뒤, 마지막으로 flood generation typing을 수행한다. 기본 worker 수는 `4`이고, 모델 학습과 동시에 돌릴 때는 서버 자원 상황에 맞춰 줄일 수 있다.
...
Model 2는 probabilistic model이므로 coverage, calibration, pinball loss도 추가로 본다. 특히 `q95`와 `q99`가 실제 홍수 첨두를 잘 감싸는지 확인해야 한다.
...
문서 기준으로 연구 설계, basin 기준, Model 1/2 config, subset300 main comparison runner는 준비되어 있다. paired seed `111 / 222 / 444` 기준 hydrograph 분석과 event-regime 분석도 진행됐고, primary extreme-rain stress test는 완료됐다.

현재 추가로 돌리는 큰 항목은 같은 극한호우 cohort에 대해 validation checkpoint grid 전체를 다시 평가하는 sensitivity run이다. 아직 남은 큰 일은 이 all-validation-epoch stress-test 결과를 정리하고, final screening table과 논문용 결과표/그림을 닫는 것이다.
