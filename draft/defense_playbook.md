...
현재 broad profile의 usability 기준은 `train 720`, `validation 168`, `test 168` valid `Streamflow` count다. 따라서 디펜스에서는 “우리는 basin을 quality와 usability 두 층으로 나눠 관리하고, 공식 상태는 `output/basin/all/screening/camelsh_basin_master_checklist_broad.csv`의 `minimum_quality_gate_pass`와 `usability_status`로 추적한다”라고 설명하는 편이 가장 정확하다.

### Q6-1. train basin 중 실제 train 기간 support가 너무 적은 basin이 있으면 어떻게 방어하나요?

이 문제는 인정하되, 실험을 무효화하는 문제로 말하지 않는다. 핵심은 `selected basin`, `split-period eligibility`, `effective sample support`를 구분하는 것이다. 어떤 basin이 train file에 포함되었다는 말은 그 basin이 fixed split membership에 들어갔다는 뜻이고, split-period eligibility는 해당 train window 안에서 minimum valid target count를 통과했다는 뜻이다. 반면 실제 학습에 기여한 양은 sample-level support, 즉 `valid_model_sample_count`로 따로 봐야 한다.

현재 완료된 main comparison은 재학습하지 않는다. 대신 post-hoc support diagnostic으로 방어한다. [`../../output/basin/timeseries/train_support_diagnostics_summary.json`](../../output/basin/timeseries/train_support_diagnostics_summary.json)에 따르면 fixed `scaling_300` train split에서 `used_coverage_fraction <= 0.10`인 basin은 `03057000` 하나이고, estimated valid training sample은 `2,180`개다. 전체 train valid sample `21,017,096`개 중 약 `0.0104%`이므로, 이 basin은 membership에는 포함되었지만 sample-level contribution은 매우 작다. `used_coverage_fraction <= 0.25`로 넓혀도 `9`개 basin이 전체 sample의 약 `0.73%`만 차지한다.

따라서 방어 문장은 이렇게 잡는 것이 안전하다. “Some selected train basins have sparse target availability within the calendar train window. We therefore distinguish split membership from effective sample support and report post-hoc support diagnostics. Sparse-support basins passed the predefined split-period minimum valid target gate, but contributed proportionally few training samples. Because Model 1 and Model 2 use the identical fixed split and paired seeds, this limitation affects absolute data support rather than the fairness of the model comparison.”

주의할 점은 `low-support basin을 제거했더니 결과가 같다`고 말하면 안 된다는 것이다. 제거 후 재학습을 하지 않았기 때문이다. 대신 말할 수 있는 것은 `low-support basin의 학습 sample 비중이 작다`, `두 모델 모두 같은 split을 공유한다`, `test/validation metric support는 별도 coverage table로 보고한다`까지다.
...
### Q10-1. Q99 event가 진짜 홍수인가요, 아니면 그냥 유량이 조금 높았던 건가요?

Q99 event를 곧바로 official flood라고 부르면 안 된다. 정확한 표현은 `basin-specific observed-streamflow Q99 high-flow event candidate`다. 여기서 Q99는 강수량 threshold가 아니라 관측 유량 threshold이므로, 비가 많이 왔지만 유량 반응이 없었던 경우는 event로 잡히지 않는다. 반대로 유량 상위 1%라고 해서 반드시 flood stage 초과나 피해 발생을 뜻하지도 않는다.

그래서 우리 절차는 두 단계로 설명해야 한다. 첫째, Q99/Q98/Q95로 observed high-flow candidate를 넓게 잡는다. 둘째, 그 candidate의 flood-like severity를 `unit_area_peak`, annual peak relevance, `peak_to_flood_ari*` ratio, rising rate, event duration으로 다시 해석한다. return-period reference가 있으면 `flood_relevance_tier`를 붙여 `high_flow_below_2yr_proxy`와 `flood_like_ge_2yr_proxy` 이상을 구분한다.

추가 defense로는 `Q99-only`, fallback 포함 전체, `return_period_confidence_flag`별 sensitivity를 같이 제시한다. 이 세 결과에서 Model 1 / Model 2의 방향이 유지되면, event threshold가 결론을 만든 것이 아니라는 설명이 가능하다.

방어 문장은 “Q99 is an event detection threshold, not a claim of official flood occurrence. We use it to identify observed high-flow response candidates and then characterize their flood relevance using peak magnitude, area-normalized discharge, hydrograph shape, and return-period proxy ratios” 정도가 안전하다.
...
조금 더 구체적으로 말하면, Merz and Blöschl (2003)는 annual flood peak를 hydrometeorological indicator와 catchment state로 process type에 연결했고, Stein et al. (2020)은 precipitation, simple soil moisture, snowmelt 기반 decision tree로 event type을 만들었고, Jiang et al. (2022)은 explainable ML과 clustering으로 annual maximum event를 `recent precipitation`, `antecedent precipitation`, `snowmelt` 세 mechanism으로 해석했다. 즉 선행연구의 공통 절차는 `event definition -> event driver descriptor -> process label -> basin-level proportion/dominant type`이다.

우리도 그 절차를 따른다. 다만 CAMELSH hourly에서 안정적으로 얻을 수 있는 값이 precipitation, temperature, streamflow이므로, soil moisture와 SWE를 직접 쓰지 않는다. 대신 rule-based QA label에서는 `antecedent_rain_7d/30d`를 wetness proxy로 두고, snow 쪽은 `Tcrit = 1°C`, degree-day factor `2.0 mm/day/°C`의 event-window snowmelt proxy를 계산한다. 그래서 label도 `snowmelt_or_rain_on_snow`처럼 보수적으로 합친다. 모델 오차 stratification의 주 분석 축은 이 rule label이 아니라, `hydromet_only_7 + KMeans(k=3)` ML-based event-regime cluster다.

방어 문장은 “We do not claim to observe snowmelt directly. We classify snow-related events using a literature-aligned degree-day proxy derived from event-window precipitation and temperature, and interpret the result as a snowmelt/rain-on-snow proxy rather than confirmed snowmelt occurrence” 정도가 안전하다.

이 분류는 `학습 전 screening`이 아니라 `평가 후 해석` 계층에 둬야 한다. 그렇지 않으면 typing 자체가 또 하나의 screening criterion이 되어 설계가 너무 무거워진다. 현재 ML cluster도 causal mechanism을 확정한 label이 아니라 descriptor-space event regime이므로, type별 결론을 강하게 쓰려면 `Q99-only`, fallback 포함 전체, rule-based `degree_day_v2` sensitivity, `uncertain_high_flow_candidate` 포함/제외를 같이 확인한다. 방어 문장은 “we do not claim a new universal flood typology; we use a literature-aligned event-first rule label as QA and a data-driven event-regime clustering to stratify model errors” 정도가 가장 안전하다.
...
현재 저장소에는 broad config와 subset300 실행 스크립트, prepared split, split manifest가 정리돼 있다. 현재 로컬에는 Model 1 seed `111 / 222 / 333 / 444`, Model 2 seed `111 / 222`, 그리고 NaN loss로 중단된 Model 2 seed `333` 산출물이 있다. 논문용 공식 비교 결과표는 Model 1 / Model 2 모두 seed `111 / 222 / 444`로 맞춰 만들고, 완료된 Model 1 seed `333`은 paired-seed fairness를 위해 final aggregate에서 제외한다고 설명하는 것이 정확하다.

### Q30. 그러면 논문 baseline은 무엇으로 설명해야 하나요?

논문 baseline은 `fixed subset300 main comparison`으로 설명하면 된다. broad config와 prepared split은 여전히 source pool과 reference architecture context를 제공하지만, 현재 본 실험의 직접 실행 기준은 [`../../configs/pilot/basin_splits/scaling_300/`](../../configs/pilot/basin_splits/scaling_300/)과 [`../../scripts/runs/official/run_subset300_multiseed.sh`](../../scripts/runs/official/run_subset300_multiseed.sh)다.
...
둘째, `multi-seed protocol`을 고정해야 한다. 적어도 model별 3-repeat 평균은 있어야 한다.
...
- Jiang et al. (2022), *River flooding mechanisms and their changes in Europe revealed by explainable machine learning*. event-first, basin-summary flood generation typing과 data-driven event grouping의 큰 방향을 제공한다. [HESS](https://hess.copernicus.org/articles/26/6339/2022/)
- Stein et al. (2020), *Event-based classification for global study of river flood generating processes*. precipitation, soil moisture, snowmelt 기반 decision tree로 event별 `short rain`, `long rain`, `excess rainfall`, `snowmelt`, `rain/snow` type을 분류했다. 우리 `degree_day_v2` rule-based QA label의 가장 가까운 절차적 근거다. [PDF](../references/analysis/flood_generation_typing_literature/stein_2020_hyp_event_based_classification_global_flood_generating_processes.pdf)
- Tarasova et al. (2019), *Causative classification of river flood events*. flood causative classification review로, hydroclimatic, hydrological, hydrograph-based 접근을 정리한다. 우리 방법이 event causation literature의 어느 위치에 있는지 설명할 때 쓴다. [PDF](../references/analysis/flood_generation_typing_literature/tarasova_2019_wires_causative_classification_river_flood_events.pdf)
- Berghuijs et al. (2016), *Dominant flood generating mechanisms across the United States*. extreme rainfall만으로 flood response를 설명하기 어렵고 soil moisture, snowmelt, rain-on-snow가 중요하다는 미국권 근거다. [PDF](../references/analysis/flood_generation_typing_literature/berghuijs_2016_grl_dominant_flood_generating_mechanisms_us.pdf)
...
- [`../experiment/method/model/architecture.md`](../experiment/method/model/architecture.md): 현재 논문 범위의 backbone과 head 구조, future-work core 메모
- [`../experiment/method/model/experiment_protocol.md`](../experiment/method/model/experiment_protocol.md): split, loss, metric, config key 규칙
- `output/basin/all/screening/camelsh_basin_master_checklist_broad.csv`: broad profile의 공식 basin checklist
...
- [`../../scripts/runs/official/run_subset300_multiseed.sh`](../../scripts/runs/official/run_subset300_multiseed.sh): 현재 공식 compute-constrained main comparison runner
