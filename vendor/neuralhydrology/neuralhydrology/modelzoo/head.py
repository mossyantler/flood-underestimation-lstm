...
def get_head(cfg: Config, n_in: int, n_out: int) -> nn.Module:
    ...
    elif cfg.head.lower() == "quantile":
        head = Quantile(n_in=n_in, n_out=n_out, quantiles=cfg.quantiles)
...
class Quantile(nn.Module):
    ...
    This head predicts a point estimate and an ordered set of quantiles for each target variable. The first quantile
    acts as the base prediction and all following quantiles are modeled as positive increments on top of it, which
    ...
    n_in : int
        Number of input neurons.
    n_out : int
        Number of output neurons. Corresponds to ``n_targets * len(quantiles)``.
    quantiles : list[float]
        Sorted list of quantiles. Must contain ``0.5`` because ``y_hat`` is taken from the median prediction.
    """

    def __init__(self, n_in: int, n_out: int, quantiles: list[float]):
        super(Quantile, self).__init__()

        if not quantiles:
            raise ValueError("Quantile head requires a non-empty `quantiles` config entry.")
        ...
        if 0.5 not in quantiles:
            raise ValueError("Quantile head requires 0.5 in `quantiles` so `y_hat` can map to the median.")

        self._n_quantiles = len(quantiles)
        if n_out % self._n_quantiles != 0:
            raise ValueError("Quantile head output size must be divisible by the number of quantiles.")

        self._n_targets = n_out // self._n_quantiles
        self._median_idx = quantiles.index(0.5)
        self._projection = nn.Linear(n_in, n_out)
        self._softplus = nn.Softplus()

    def forward(self, x: torch.Tensor) -> Dict[str, torch.Tensor]:
        """Perform a forward pass on the Quantile head."""
        raw = self._projection(x)
        raw = raw.view(*raw.shape[:-1], self._n_targets, self._n_quantiles)
        ...
        y_quantiles = quantiles.reshape(*quantiles.shape[:-2], self._n_targets * self._n_quantiles)

        return {'y_hat': y_hat, 'y_quantiles': y_quantiles}
