...
class MaskedQuantileLoss(BaseLoss):
    """Weighted pinball loss for quantile regression.

    To use this loss in a forward pass, the passed `prediction` dict must contain the key ``y_quantiles``, and the
    `data` dict must contain ``y``.
    ...
    def __init__(self, cfg: Config):
        super(MaskedQuantileLoss, self).__init__(cfg,
                                                prediction_keys=['y_quantiles'],
                                                ground_truth_keys=['y'],
                                                output_size_per_target=len(cfg.quantiles))

        weights = cfg.quantile_loss_weights
        if weights is None:
            weights = [1.0 for _ in cfg.quantiles]
        if len(weights) != len(cfg.quantiles):
            raise ValueError("Number of `quantile_loss_weights` must match the number of `quantiles`.")

        self._quantiles = torch.tensor(cfg.quantiles, dtype=torch.float32)
        self._weights = torch.tensor(weights, dtype=torch.float32)

    def _get_loss(self, prediction: Dict[str, torch.Tensor], ground_truth: Dict[str, torch.Tensor], **kwargs):
        pred = prediction['y_quantiles']
        target = ground_truth['y']
        mask = ~torch.isnan(target).squeeze(-1)

        quantiles = self._quantiles.to(pred.device).view(1, 1, -1)
        weights = self._weights.to(pred.device).view(1, 1, -1)
        errors = target.expand_as(pred) - pred
        pinball = torch.maximum(quantiles * errors, (quantiles - 1.0) * errors)

        weighted_pinball = (pinball * weights).sum(dim=-1) / weights.sum()
        return torch.mean(weighted_pinball[mask])
