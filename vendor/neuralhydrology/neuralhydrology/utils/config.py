...
class Config(object):
    ...
    def quantiles(self) -> List[float]:
        quantiles = self._as_default_list(self._cfg.get("quantiles", []))
        if self.head.lower() == "quantile":
            if not quantiles:
                raise ValueError("`quantiles` must be specified when using the quantile head.")
            ...
            if 0.5 not in quantiles:
                raise ValueError("`quantiles` must contain 0.5 so the quantile head can expose `y_hat`.")
        return quantiles

    @property
    def quantile_loss_weights(self) -> List[float]:
        return self._cfg.get("quantile_loss_weights", None)
