...
def get_tester(cfg: Config, run_dir: Path, period: str, init_model: bool) -> BaseTester:
    ...
    -------
    BaseTester
        `RegressionTester` if the model head is one of {'regression', 'quantile'}. `UncertaintyTester` if the model
        head is one of {'gmm', 'cmal', 'umal'} or if the evaluation is run in MC-Dropout mode.
    """
    ...
    elif cfg.head.lower() in ["regression", "quantile", ""]:
        Tester = RegressionTester
    else:
        raise NotImplementedError(f"No evaluation method implemented for {cfg.head} head")
