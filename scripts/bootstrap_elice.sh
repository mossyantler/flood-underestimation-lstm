...
#
# 사용법 (서버 터미널에서):
#   curl -fsSL https://raw.githubusercontent.com/mossyantler/flood-underestimation-lstm/main/scripts/bootstrap_elice.sh | bash
#
# 또는 repo를 이미 clone한 경우:
#   bash scripts/bootstrap_elice.sh [--skip-data] [--model {1|2|both}]
#
# 옵션:
...
if [ ! -f "scripts/bootstrap_elice.sh" ]; then
  echo ""
...
")
  ...
  python scripts/prepare_camelsh_generic_dataset.py \
    --profile broad \
