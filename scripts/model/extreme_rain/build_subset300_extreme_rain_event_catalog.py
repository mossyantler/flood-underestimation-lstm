...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--wet-rain-threshold-mm-h",
        type=float,
        default=0.5,
        help="Minimum hourly Rainf used to define wet-footprint clusters.",
    )
    parser.add_argument(
        "--wet-gap-hours",
        type=int,
        default=12,
        help="Maximum dry gap allowed inside one wet-footprint cluster.",
    )
...
def iter_wet_clusters(wet: pd.Series, gap_hours: int) -> list[tuple[pd.Timestamp, pd.Timestamp]]:
    times = list(pd.to_datetime(wet.index[wet.fillna(False)]))
    if not times:
        return []
    clusters: list[tuple[pd.Timestamp, pd.Timestamp]] = []
    start = previous = times[0]
    for time in times[1:]:
        dry_gap = max(0.0, (time - previous).total_seconds() / 3600.0 - 1.0)
        if dry_gap <= gap_hours:
            previous = time
            continue
...
def selected_trigger_intervals(
    *,
    event_start: pd.Timestamp,
    event_end: pd.Timestamp,
    ratio_by_period_duration: dict[int, dict[int, pd.Series]],
    near_ari100_ratio: float,
) -> list[tuple[pd.Timestamp, pd.Timestamp, int, int, float]]:
    endpoint_index = ratio_by_period_duration[PRECIP_PERIODS[0]][PRECIP_DURATIONS[0]].loc[event_start:event_end].index
    intervals: list[tuple[pd.Timestamp, pd.Timestamp, int, int, float]] = []
    for endpoint in endpoint_index:
        for period in PRECIP_PERIODS:
            for duration in PRECIP_DURATIONS:
                ratio = ratio_by_period_duration[period][duration]
                value = safe_float(ratio.get(endpoint, math.nan))
                if value >= 1.0 or (period == 100 and value >= near_ari100_ratio):
                    start = pd.Timestamp(endpoint) - pd.Timedelta(hours=duration - 1)
                    intervals.append((start, pd.Timestamp(endpoint), period, duration, value))
    return intervals


def wet_footprint_times(
    *,
    rain: pd.Series,
    event_start: pd.Timestamp,
    event_end: pd.Timestamp,
    rolling_peak_time: pd.Timestamp,
    ratio_by_period_duration: dict[int, dict[int, pd.Series]],
    args: argparse.Namespace,
) -> dict[str, Any]:
    intervals = selected_trigger_intervals(
        event_start=event_start,
        event_end=event_end,
        ratio_by_period_duration=ratio_by_period_duration,
        near_ari100_ratio=args.near_ari100_ratio,
    )
    envelope_start = min((item[0] for item in intervals), default=event_start)
    envelope_end = max((item[1] for item in intervals), default=event_end)
    envelope_rain = rain.loc[envelope_start:envelope_end].fillna(0.0)
    wet = envelope_rain > args.wet_rain_threshold_mm_h
    clusters = iter_wet_clusters(wet, args.wet_gap_hours)
    if clusters:
        scored = []
        for start, end in clusters:
            segment = rain.loc[start:end].fillna(0.0)
            scored.append(
                {
                    "start": start,
                    "end": end,
                    "total": float(segment.sum()),
                    "peak": float(segment.max(skipna=True)) if len(segment) else 0.0,
                }
            )
        best = sorted(scored, key=lambda row: (-row["total"], -row["peak"], row["start"]))[0]
        footprint_start = best["start"]
        footprint_end = best["end"]
    else:
        footprint_start = envelope_start
        footprint_end = envelope_end

    footprint_rain = rain.loc[footprint_start:footprint_end].fillna(0.0)
    if footprint_rain.empty:
        footprint_peak = rolling_peak_time
        footprint_total = math.nan
        footprint_peak_value = math.nan
    else:
        footprint_peak = pd.Timestamp(footprint_rain.idxmax())
        footprint_total = float(footprint_rain.sum())
        footprint_peak_value = float(footprint_rain.max(skipna=True))

    return {
        "rain_start": footprint_start,
        "rain_peak": footprint_peak,
        "rain_end": footprint_end,
        "rolling_envelope_start": envelope_start,
        "rolling_envelope_end": envelope_end,
        "wet_cluster_total_rain": footprint_total,
        "wet_cluster_peak_rainf": footprint_peak_value,
    }
    ...
    *,
    ...
    for event_index, (event_start, event_end) in enumerate(active_events, start=1):
        ...
        if peak_metric.notna().any():
            rolling_peak = pd.Timestamp(peak_metric.idxmax())
        else:
            rolling_peak = pd.Timestamp(event_slice["max_prec_ari25_ratio"].idxmax())
        if args.event_time_mode == "wet_footprint":
            event_times = wet_footprint_times(
                rain=rain,
                event_start=event_start,
                event_end=event_end,
                rolling_peak_time=rolling_peak,
                ratio_by_period_duration=ratio_by_period_duration,
                args=args,
            )
        else:
            event_times = {
                ...
                "rain_peak": rolling_peak,
                "rain_end": event_end,
                "rolling_envelope_start": event_start,
                "rolling_envelope_end": event_end,
                "wet_cluster_total_rain": math.nan,
                "wet_cluster_peak_rainf": math.nan,
            }
        rain_start = pd.Timestamp(event_times["rain_start"])
        rain_peak = pd.Timestamp(event_times["rain_peak"])
        rain_end = pd.Timestamp(event_times["rain_end"])
        ...
        row: dict[str, Any] = {
            ...
            "rolling_endpoint_start": event_start,
            "rolling_endpoint_peak": rolling_peak,
            "rolling_severity_peak_time": rolling_peak,
            "rolling_endpoint_end": event_end,
            "rolling_envelope_start": event_times["rolling_envelope_start"],
            "rolling_envelope_end": event_times["rolling_envelope_end"],
            "rain_start": rain_start,
            "rain_peak": rain_peak,
            "rain_end": rain_end,
            "wet_cluster_total_rain": event_times["wet_cluster_total_rain"],
            "wet_cluster_peak_rainf": event_times["wet_cluster_peak_rainf"],
            "wet_rain_threshold_mm_h": args.wet_rain_threshold_mm_h,
            "wet_gap_hours": args.wet_gap_hours,
            "water_year": water_year(rain_peak),
            "peak_month": int(rain_peak.month),
            "rain_event_n_hours": int(len(rain.loc[rain_start:rain_end])),
            "rolling_endpoint_event_n_hours": int(len(event_slice)),
            "rain_coverage": rain_coverage,
        ...
        response_end = rain_end + pd.Timedelta(hours=args.response_post_hours)
        ...
        if pd.notna(response_row.get("observed_response_peak_time")):
            observed_peak_time = pd.Timestamp(response_row["observed_response_peak_time"])
            lag_from_peak = (observed_peak_time - rain_peak).total_seconds() / 3600.0
            response_row["response_lag_hours"] = lag_from_peak
            response_row["response_lag_from_rain_peak_h"] = lag_from_peak
            response_row["response_lag_from_rain_start_h"] = (observed_peak_time - rain_start).total_seconds() / 3600.0
            if observed_peak_time < rain_start:
                response_row["temporal_alignment_flag"] = "pre_rain_start"
            elif observed_peak_time < rain_peak:
                response_row["temporal_alignment_flag"] = "pre_rain_peak"
            else:
                response_row["temporal_alignment_flag"] = "aligned_after_rain_peak"
        else:
            response_row["response_lag_from_rain_peak_h"] = math.nan
            response_row["response_lag_from_rain_start_h"] = math.nan
            response_row["temporal_alignment_flag"] = "unrated"
