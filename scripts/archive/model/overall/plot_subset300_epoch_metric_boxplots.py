...
DEFAULT_PRIMARY_SUMMARY = (
    REPO_ROOT
    / "output/model_analysis/legacy/overall_analysis/main_comparison/tables/primary_epoch_summary.csv"
)
...
def main() -> None:
    ...
    metadata_dir = args.metadata_dir if args.metadata_dir.is_absolute() else REPO_ROOT / args.metadata_dir
    ...
    manifest_path = metadata_dir / "epoch_metric_boxplot_manifest.csv"
    manifest.to_csv(manifest_path, index=False)
    for split, split_manifest in manifest.groupby("split", sort=True):
        split_manifest.to_csv(metadata_dir / f"{split}_epoch_metric_boxplot_manifest.csv", index=False)
    ...
    metadata = {
        ...
        "metadata_dir": str(metadata_dir.relative_to(REPO_ROOT)),
        "splits": splits,
        "seeds": seeds,
    ...
    metadata_path = metadata_dir / "epoch_metric_boxplot_metadata.json"
    metadata_path.write_text(json.dumps(metadata, indent=2), encoding="utf-8")
    for split in splits:
        split_metadata = {**metadata, "splits": [split]}
        (metadata_dir / f"{split}_epoch_metric_boxplot_metadata.json").write_text(
