...
return_period_cmd=(
  "$UV_BIN" run scripts/basin/all/build_camelsh_return_period_references.py
...
event_response_cmd=(
  "$UV_BIN" run scripts/basin/all/build_camelsh_event_response_table.py
  --timeseries-dir "$TIMESERIES_DIR"
  --timeseries-csv-dir "$TIMESERIES_CSV_DIR"
...
if should_run "$RUN_TYPING"; then
  "$UV_BIN" run scripts/basin/all/build_camelsh_flood_generation_typing.py \
