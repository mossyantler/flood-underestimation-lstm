...
CONFIG_FILE="$ROOT_DIR/configs/camelsh_hourly_model2_drbc_holdout_broad.yml"
TMP_CONFIG=""
...
if [ -n "${NH_SEED:-}" ] || [ -n "${NH_DEVICE:-}" ] || [ -n "${NH_EXPERIMENT_SUFFIX:-}" ]; then
  TMP_CONFIG="$(mktemp "${TMPDIR:-/tmp}/camelsh_model2_broad.XXXXXX.yml")"
...
echo "Running Model 2 broad config: $CONFIG_FILE"
