...
def plot_event(
    ...
    *,
    ...
    ax_rain.axvline(pd.Timestamp(event["rain_peak"]), color="#1d4ed8", linewidth=1.0, linestyle="--")
    rain_ylim = finite_max(rain) * 1.18 if np.isfinite(finite_max(rain)) and finite_max(rain) > 0 else 1.0
    ...
    ax_flow.legend(dedup_handles, dedup_labels, loc="upper right", fontsize=8, frameon=False, ncol=2)
...
<head>
  <meta charset="utf-8">
  <title>Subset300 extreme-rain event plots</title>
  <style>
...
<body>
  <h1>Subset300 extreme-rain event plots</h1>
  <p class="meta">{len(manifest)} event plots. 이 index는 후보 진입 이유인 rainfall severity 기준으로 먼저 묶고, 그 안에서 observed streamflow response class를 나눕니다. Top panel: hourly Rainf. Bottom panel: observed Streamflow and response thresholds.</p>
  <table>
...
```text
response window = rain_start - 24h  ~  rain_end + 168h
```
...
각 PNG의 위쪽 panel은 hourly `Rainf`다. 파란 막대가 시간별 강수량이고, 진한 파란 음영은 rain event 본 구간, 옅은 파란 음영은 rain event 주변 padding이다. 파란 점선은 rain peak 시간이다.

아래쪽 panel은 observed `Streamflow`다. 검은 선이 관측 유량이고, 빨간 점이 response window 안에서 찾은 observed peak다. 연한 주황색 음영은 response window다. 회색 점선은 `Q99`, 주황 점선은 `flood_ari2`, 빨간 점선은 `flood_ari25`, 짙은 적갈색 점선은 `flood_ari100`이다.
