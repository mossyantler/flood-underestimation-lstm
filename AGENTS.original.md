...
- 응답, `docs/`, `report/` 본문은 한국어를 기본으로 작성한다.
- API, LSTM, SHAP, quantile, calibration, coverage, RandomForest, surrogate model처럼 연구·개발에서 의미가 고정된 전문 용어는 영어 그대로 사용할 수 있다. 다만 전문 용어가 아닌 일반 설명어, 문장 연결, 해석 문장은 되도록 한국어로 쓴다.
- 전문 용어를 처음 쓰거나 독자가 헷갈릴 수 있는 경우에는 짧은 한국어 설명을 붙인다. 예를 들어 `coverage`는 "관측값이 예측 quantile 아래에 들어오는 비율"처럼 설명한다.
- `docs/` 또는 `report/`를 작성할 때는 대학생 수준에서 이해할 수 있도록 쓴다. 먼저 "무엇을 보는 분석인지", 다음으로 "왜 필요한지", 마지막으로 "결과를 어떻게 해석해야 하는지" 순서로 설명한다.
- 수식, 지표, 모델 구조를 설명할 때는 정의만 나열하지 말고 직관적인 의미를 함께 쓴다. 단, 논문용 문장에서는 과장된 표현이나 인과가 확인되지 않은 단정은 피한다.
...
| `dashboard/` | React 기반 실험 분석 대시보드와 UI snapshot asset | 분석 source-of-truth는 계속 `output/`, `docs/experiment/analysis/`, `configs/`에 두고, dashboard는 표시용 UI와 필요한 snapshot asset만 둔다. 디자인은 `vercel/DESIGN.md`를 따른다. |
| `database/` | PostgreSQL/DuckDB 분석 cache schema, import helper, local database artifact 경로 | 원본 source-of-truth를 대체하지 않는다. 반복 join이 필요한 summary는 PostgreSQL, 큰 CSV/Parquet 탐색은 DuckDB를 쓰며 `database/local/` 생성물은 git에 올리지 않는다. |
...
PostgreSQL과 DuckDB는 데이터 확인, JOIN, sanity check, DBeaver grid 탐색의 1차 조회 기준으로 사용할 수 있다. 특히 basin 속성, event response/regime, model metric, probabilistic diagnostics를 함께 비교할 때는 `analysis.*` typed table과 DuckDB view를 먼저 조회해도 된다.

단, 데이터 내용 수정의 기준은 여전히 원본 artifact와 generator script다. DB 값을 직접 `UPDATE`해서 canonical data를 고치지 말고, 이상값을 발견하면 `analysis.csv_files`의 `relative_path`, `sha256`, `imported_at`로 원본을 확인한 뒤 `output/`, `configs/`, `data/`의 원본 CSV 또는 해당 CSV를 만든 script를 수정하고 importer를 다시 실행한다.

DB cache 구조 자체를 바꾸는 경우, 예를 들어 schema, importer, migration, DuckDB view를 수정하는 작업은 `database/`에서 처리한다. 이 경우에도 `database/local/` 생성물은 git에 올리지 않고, 값의 의미나 해석을 바꾸는 변경이면 관련 `docs/experiment/analysis/` 또는 source generator 문서까지 함께 갱신한다.
