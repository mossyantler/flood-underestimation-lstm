...
- GAGES-II basin geometry를 지도 렌더링에 사용할 때는 `EPSG:5070`(NAD83 / Conus Albers)을 기준 CRS로 사용한다. USGS `gagesii-basins` API/cache처럼 CRS provenance가 있는 geometry를 우선 사용하고, `.prj` 없는 로컬 GAGES-II boundary도 `EPSG:5070` 기준으로 처리한다. 렌더링·clip·거리/면적/overlap 계산은 DRBC boundary와 함께 `EPSG:5070`에서 수행하고, figure manifest에 입력 출처·최종 CRS·cache 경로를 남긴다.
